...
**{{ productName }}** позволяет отправлять HTTP-запросы типов `GET`, `POST`, `PUT`, `DELETE` для взаимодействия с внешними системами. Здесь представлены инструкции по настройке подключения для отправки HTTP-запросов из сценариев с помощью [действия «**Отправить сообщение**»](https://kb.comindware.ru/article.php?id=4718#scenario_actions_send_message).
...
- *[Отправка HTTP-запросов типа GET. Пример: настройка подключения, пути передачи данных и сценария для обработки запросов в формате JSON](https://kb.comindware.ru/article.php?id=4699)*
- *[Действие «Отправить сообщение» в сценарии](https://kb.comindware.ru/article.php?id=4718#scenario_actions_send_message)*
