...
- Для каждого аккаунта в **{{ productName }}** необходимо задать следующие атрибуты:
  - **имя пользователя**;
  - **адрес эл. почты**.
- Значения этих атрибутов **должны быть уникальны** в рамках экземпляра **{{ productName }}**.
...
- **Настройка с помощью {{ productName }}** — необходимо задать:
...
Для настройки замещений, они должны быть включены в конфигурации **{{ productName }}**.
...
Чтобы добавить аккаунту дополнительные атрибуты, изначально не предусмотренные в **{{ productName }}**, его следует привязать к [шаблону аккаунта](https://kb.comindware.ru/article.php?id=4757).
...
Подробные сведения о лицензировании **{{ productName }}** см. в разделе «**[Лицензирование»](https://kb.comindware.ru/article.php?id=4670)**.
