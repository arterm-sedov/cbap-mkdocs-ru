...
2. Отобразится конструктор диаграммы процесса.
   ...
   **(2) Панель инструментов**
   ...
   - **Экспорт** *‌* — экспорт диаграммы в формате `BPMN`, `PNG` или `SVG`, а также экспорт регламента процесса в формате `DOCX` или `PDF` (может требоваться лицензия на **{{ productName }} Enterprise**). См. *«[Импорт и экспорт диаграмм и регламентов](https://kb.comindware.ru/article.php?id=4832)»*.
