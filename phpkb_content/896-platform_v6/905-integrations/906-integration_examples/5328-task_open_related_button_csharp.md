...
- если задаче назначено **несколько возможных исполнителей**, пользователь **должен принять задачу в работу**. **Только принятая в работу** задача будет назначена пользователю как активная. См. *«[Кнопка «Принять в работу» для задачи. Настройка на C#][example_task_accept_button_csharp]»*
