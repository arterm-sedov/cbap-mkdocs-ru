...
Если внешняя система передаёт данные не через брокер сообщений, а прямым HTTP-запросом, используйте *«[Адаптер для импорта данных. Настройка подключения, пути передачи данных и сценария][adapter_data_import]»*. Этот адаптер принимает HTTP-запрос, преобразует XML-сообщение во внутреннюю структуру DataImport и запускает сценарий обработки сообщения.

Если требуется отправлять данные из **{{ productName }}** в топик Apache Kafka, используйте *«[Адаптер экспорта данных из платформы. Настройка подключения, пути передачи данных и сценария][adapter_data_export]»*.
...
- При получении сообщений через шину Apache Kafka **{{ productName }}** является **потребителем**, а при отправке — **продюсером**.
...
Для получения сообщений через шину Apache Kafka и передачи данных в шаблон записи требуется настроить [сценарий][scenarios], срабатывающий при поступлении сообщения из Apache Kafka.
