...
- **Синхронизация с сервером каталогов** — **на стороне сервера каталогов** необходимо задать значения следующих атрибутов (они не должны быть пустыми и не должны иметь значение `Null`):
  ...
  См. *«[Синхронизация с сервером каталогов (Active Directory)][accounts_dc_sync]»*.
...
См. *«[Замещения. Настройка конфигурации][substitution_configuration]»*.
...
Если у пользователя есть разрешение на просмотр аккаунтов (заданное с помощью [системной роли][system_roles]), то он сможет аналогичным образом настроить для себя замещение на странице «**Мой аккаунт**».
