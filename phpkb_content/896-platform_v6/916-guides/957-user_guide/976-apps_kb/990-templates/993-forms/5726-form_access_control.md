...
kbId: 5726
url: 'https://kb.comindware.ru/article.php?id=5726'
...
Рассмотрим пример настройки ссылки на внешнюю форму *«Клиент»* из *[предыдущего примера](#form_access_control_configure)*, чтобы любой клиент мог оставлять заявку, не выполняя вход в **{{ productName }}**.

Логика работы внешних форм

Если создана ссылка на внешнюю форму, она действует следующим образом:
...
- `ObjectTypeId` — `строка` вида `oa.XXX`, ID шаблона записи, к которому относится форма;
- `ObjectId` — `строка` вида `XXX`, ID записи (**необязательно**);
...
См. *«[Аутентификация для доступа к API][api_intro]»* и *«[Методы System Core API][api_system_core]»*.
...
- [Аутентификация для доступа к API][api_intro]
