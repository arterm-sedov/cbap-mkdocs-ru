...
| [Стандарты редактирования](#стандарты-редактирования-контента) | Те же правила, что у агентов; см. `AGENTS.md` |
| [Устранение неполадок](#устранение-неполадок) | Типичные ошибки и их устранение |

**Пишете или правите `docs/ru/`?** → [`AGENTS.md`](AGENTS.md). **Запускаете сборки, импорт или скрипты?** → разделы ниже (индекс навыков (*skills*) также в [`AGENTS.md` → Справочник навыков](AGENTS.md#skills-reference)).

## Стандарты редактирования контента

У людей и агентов один свод правил: [`AGENTS.md`](AGENTS.md). Перед правкой статьи откройте нужные разделы:

| Тема | Где в `AGENTS.md` |
| --- | --- |
| Внешние ссылки и ссылки на БЗ, карта гиперссылок | [Оформление ссылок](AGENTS.md#link-formatting) |
...
Условия Jinja и `{% include-markdown %}` — в [Основах Jinja](#основы-jinja) (поведение при сборке). Стиль ссылок и формулировок — в `AGENTS.md`.
...
MkDocs обрабатывает Markdown через Jinja2. Плагин `mkdocs-macros` (`plugins.macros` в `mkdocs_common.yml`) открывает значения `config.extra` как переменные шаблона. Плейсхолдеры и условия должны соответствовать [`AGENTS.md`](AGENTS.md) (стиль ссылок, полужирные имена продуктов и т.&nbsp;д.) — ниже только механика сборки.
...
Переиспользуемые фрагменты — в `docs/ru/.snippets/`. Подключение в конце статьи:

```markdown
{% include-markdown ".snippets/hyperlinks_mkdocs_to_kb_map.md" %}
```

Карта гиперссылок — **единый источник** внешних и KB-ссылок. В экспорте PHPKB блок для БЗ внутри карты обёрнут в `{% if kbExport %} … {% endif %}`, чтобы PDF и веб-сборки не дублировали URL.

### Ссылки в статьях

- Внешние / БЗ: `[текст][anchor]` — якорь в `hyperlinks_mkdocs_to_kb_map.md`.
- Внутри страницы: `[текст](#article_anchor)`.
- Голые `https://` в статьях не использовать — сначала добавьте цель в карту гиперссылок.
...
Полные сценарии: `.agents/skills/<name>/SKILL.md`. **ИИ-агенты:** индекс в [`AGENTS.md`](AGENTS.md) — подключайте навык (*skill*), когда его описание подходит задаче; не опирайтесь на это readme для правил написания и шагов навыков.

Люди могут читать навыки как расширенные инструкции. Частые записи:
