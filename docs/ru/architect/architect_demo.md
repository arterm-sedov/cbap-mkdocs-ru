...
На демонстрационном стенде представлены базовые элементы для создания процессной архитектуры и организационно-функциональной структуры (ОФС).
...
- **Введение в конструктор корпоративной архитектуры**
    ...
    - Организационно-функциональная структура (ОФС)
    - Две роли, используемые в демонстрации:
        ...
        - _Сотрудник_ занимается просмотром моделей процессов, ОФС и выполнением задач, связанных с согласованием и аудитом
...
- **Формирование ОФС**
    - Формирование ОФС, заполнение справочников и установление связей
...
_Архитектор_ занимается построением процессной архитектуры и организационно-штаной структуры (ОФС), наполнением и созданием справочников, связанных с корпоративной архитектурой:
...
- **Оргструктура**, **Процессы** — реестры ОФС и бизнес-процессов.
...
- **С чего начать** — виджет с типовыми операциями, которые выполняет _Архитектор_:
    ...
    1. Формирование ОФС и заполнение справочников:
    ...
    6. Анализ архитектуры:
        ...
        - Просмотр таблиц по группам процессов, процессов, задач и ОФС с возможностью фильтрации по метаданным элементов архитектуры.
...
## Организационно-функциональная структура (ОФС) {: .pageBreakBefore }
...
### Возможности построения ОФС

В модуле «**{{ productNameArchitect }}**» предусмотрены следующие возможности построения организационно-функциональной структуры (ОФС):

- Создание ОФС структуры с использованием диаграммы.
- Визуализация как оргструктур отдельных компаний, так и сложных многоуровневых оргструктур групп компаний.
- Реструктуризация ОФС.
- Расширение элементов ОФС с использованием произвольного набора атрибутов.
- Наполнение ОФС посредством импорта файлов .xlsx, .csv или интеграции с 1С по OData.
- Настройка и формирование печатных документов — должностных инструкций, положений о подразделениях и др.

### Использование демонстрационной ОФС {: .pageBreakBefore }

На демонстрационном стенде сформирован иерархический реестр подразделений и должностей в организации — ОФС, которая содержит иерархию подразделений и должностей.
...
- **Места работы** — позволяют назначать сотрудникам должности из ОФС.
...
3. Откроется диаграмма ОФС в контексте выбранной оргединицы.
...
8. Отобразится диаграмма исполняемого процесса, показывающая движение токена, остановившегося на задаче «**Назначить аудитора**».
    ...
    !!! warning "Логика определения исполнителя задачи по выбору аудитора"
        ...
        Эта задача назначается на _Сотрудника_, занимающего должность **Главного аудитора** из нашей ОФС. 
