...
!!! warning "Бизнес-логика"
    ...
    В курсе используется контекст HR‑процесса подбора кандидатов с соответствующими сущностями: кандидаты, анкеты, отделы и т.&nbsp;п.
...
Например, структуру данных в шаблоне записи можно упрощённо представить следующим видом:
...
Соответствующие записи в шаблоне _«Сотрудники»_:
...
Теперь, когда вы ознакомились с базовым синтаксисом языка N3 и принципами работы с графовыми данными, научимся вычислять значение атрибута шаблона записи с помощью N3 и встроенных в **{{ productName }}** предикатов для обработки данных.

!!! warning "Бизнес-логика"

    Рассмотрим вычисление в N3 на следующем примере:

    В шаблоне записи _«Сотрудники»_ имеются атрибут _«Имя»_, _«Фамилия»_ и _«Отчество»_ с соответствующими системными именами.

    Требуется автоматически заполнять атрибут _«Ф.И.О.»_ составной строкой из имени, фамилии и отчества сотрудника.

!!! question "Расширенный синтаксис N3"

    В этом примере мы применим следующие элементы N3:

    - Предикаты для обработки данных — это встроенные в **{{ productName }}** функции, которые можно импортировать в выражение посредством директивы `@prefix` с URI соответствующего ресурса. Например, ресурс <http://comindware.com/ontology/object#> содержит предикаты для работы с записями и другими объектами.
...
3. В поле «**Вычисляемое значение**» введите следующее выражение на **N3**:
...

    {
        ...
        # FirstName, LastName, Patronymic для дальнейшей обработки.
        ?item ?FirstNameAttribute ?FirstName.
        ?item ?LastNameAttribute ?LastName.
        ?item ?PatronymicAttribute ?Patronymic.

        # С помощью предиката cmwstring:format соединяем
        # фамилию, имя и отчество в единую строку и помещаем
        # её в переменную FullName.
        ("{0} {1} {2}" ?LastName ?FirstName ?Patronymic) cmwstring:format ?FullName.
