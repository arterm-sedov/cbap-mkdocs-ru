...
### Global sourcing and constraints

- Always curate sources via `@n3_references.mdc`. When adding examples or definitions, pick from the referenced files there (e.g., `n3_examples_collection.md`, `n3_lecture_transcript_2025.08.21.md`, developer guide articles, and examples under `docs/ru/examples/`).
- Formulas are a separate language and are not a kind of triples. When formulas are mentioned, link to `docs/ru/developer_guide/formulas/formula_guide.md` and keep N3 content distinct.
- Platform specifics: use `->` for result assignment; implication `=>` is not used for standard expressions.
...
- Triples with visuals; core syntax preview: variables, `{}`, `.`, prefixes; brief mention of shorthands `a`, `[]`, `=`, platform `->` (full details in Lesson 2)
- Interpreter behavior preview: sequential AND and evaluation order; brief reasoner note (details in Lesson 5)
...
- N‑Triples vs N3; one‑line mapping example; optional Formula → N3 mini‑refactor preview (full in Lesson 2)
...
- Five forms/query patterns with tiny runnable snippets
- Shorthand constructs: `a` (rdf:type), `[]` (blank node pattern), `=` (binding/equality), platform `->` result; note `=>` not used
- Interpreter behavior: default conjunction (AND), sequential evaluation; emptiness checks preview (`not { … ?. }`, `?.`)
- Error hygiene: dots, braces, case sensitivity; variable reuse pitfall
- Formula → N3 refactor (simple) with a tiny before/after
- Sources:
...
- Example patterns: current user filter for responsibility; linked status → code check (e.g., Q0) via referenced entity
- Sources:
...
- Three equivalent assignment variants; exact‑match checks; when variants differ in performance
- Order matters (teaser); full performance details in Lesson 5
...
- Performance hygiene; "срез данных" (order by selectivity) with a compact example; safe `once {}` cases
- Counting: prefer `assert:count` for record counts vs `cmwnullable:count`
- Note: historical "container" caching approach is discouraged in recent builds
...
- Refactor pattern: rewrite simple IF/ELSE to two small OR branches for performance
...
- Aggregations and list utilities: `cmwlist:ascending`, `cmwlist:at`; last item via `w3list:last` or `assert:sort`
- Sources:
...
- Practical function recipes and data types (numbers vs strings: `1` vs `"1"`)
- Nullable‑safe comparisons and arithmetic
- String functions: `string:*` vs `cmwstring:*` (e.g., format vs join/contains); `string:regexReplace`
- Time/duration: `cmwlocal:*`, `cmwdur:*`, range checks (`between`)
- Sources:
...
- Group membership pattern (e.g., `account:groupName`, `userGroupMembership`)
- Processes and tasks: counting active processes; in‑progress tasks filter
...
- Session variables and local state: `session:context`, `variable`/`operator` usage; result building patterns; "empty" fallback idiom
- Attachments → hyperlink recipe (e.g., `document:title`, `document:content` + formatted link)
- Sources:
...
Separate each heading with double new line \n\n
...
- **Code blocks**: Use triple backticks with language specification for N3 examples, separate language code with a space, e.g. ``` turtle
...
- **Code highlighting**: Specify language for all code blocks (`turtle`, `yaml`, `markdown`, `sql`, etc.)
...
### Integration Points
- **Reference existing N3 docs** when available (always via `@n3_references.mdc`)
...
### Notes on formulas vs N3

- Formulas are a separate syntax scope covered in `docs/ru/developer_guide/formulas/`. Do not conflate formulas with triples; when refactoring examples are shown, keep formula snippets minimal and point to the formulas reference for details.
