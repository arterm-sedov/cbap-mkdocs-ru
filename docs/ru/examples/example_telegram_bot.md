...
# Интеграция с Telegram-ботом. Пример: настройка приложения, Telegram и кода бота {: #example_telegram_bot }

## Введение {: #example_telegram_bot_intro }
...
## Прикладная задача {: #example_telegram_bot_use_case }
...
## Алгоритм авторизации {: #example_telegram_bot_algoritm_avtorizacii }
...
!!! note "Примечание"
    ...
    На основе созданного промежуточного ПО и примера настройки взаимодействия бота с **{{ productName }}** можно в дальнейшем реализовать различные сценарии: подача заявок, заявлений на отпуск, запрос информации из системы и т.&nbsp;д.

## Предварительная настройка со стороны {{ productName }} {: #example_telegram_bot_predvaritelnaya_nastroyka_so_storony_productname }

1. Создайте шаблон аккаунта _«Sotrudniki»_ (с таким же системным именем). В этом шаблоне уже есть системные атрибуты, такие как имя, почта и т.&nbsp;д.
2. Добавьте атрибут _«ChatID»_ типа «**Текст**» (в примере ID атрибута — `op.13`).
3. Добавьте атрибут _«Proverochnyykod»_ типа «**Текст**» (в примере ID атрибута — `op.14`).
4. Создайте и настройте шаблон процесса для отправки эл.&nbsp;почты пользователю (в примере ID шаблона — `pa.3`).
    ...
    - В связанном с шаблоном процесса шаблоне записи создайте два атрибута типа «**Текст**»:
        ...
        - _Код_ (в примере ID атрибута — `op.23`) — значение этого атрибута процесс должен отправлять пользователю;
        - _Кому_ (в примере ID атрибута — `op.24`) — адрес, на который процесс должен отправлять код.

!!! note "Примечание"

    В исходном коде промежуточного ПО замените ID и системные имена атрибутов и шаблонов на фактически используемые.

## Настройка интеграции {: #example_telegram_bot_nastroyka_integracii }

1. В Telegram найдите [BotFather][telegram_botfather] и запросите у него создание бота. Подробные инструкции см. в [официальной документации Telegram][telegram_core_api].
...
10. В окне приложения нажмите кнопку _«Старт»_ и подтвердите запуск ПО.
11. Напишите в Telegram-бот сообщение, например: _Привет_.
12. Бот должен ответить: _Вы написали мне: Привет_.
...
16. Добавьте в файл `MainWindow.xaml.cs` две функции:
    ...
    - `Get()` будет делать `GET`-запрос для получения данных из шаблона аккаунта _«Sotrudniki»_. Результатом будет `JSON`-ответ.
...
18. Реализуем процесс авторизации следующим образом: если пользователь по ChatID не найден, то будем рассматривать 3 варианта:
    ...
    - во всех остальных случаях бот отвечает: _«Укажите адрес эл. почты для авторизации»_.
...
- [Отправка уведомлений с помощью Telegram][telegram_send_notification]
