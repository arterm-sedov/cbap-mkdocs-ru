---
title: 'Эл.&nbsp;почта в процессе. Отправка, получение и обработка'
kbTitle: 'Эл.&nbsp;почта в процессе. Отправка, получение и обработка. Пример: настройка подключений, путей передачи данных, диаграммы процесса и сценария'
kbId: 4691
tags:
    - письма
...
## Введение {: #process_email_configure_intro }
...
## Сценарии обработки эл.&nbsp;почты в процессе {: #process_email_configure_scenarios }
...
!!! warning "Логика идентификации писем"
...
!!! tip "Технический процесс для обработки входящих писем"
...
## Прикладная задача {: #process_email_configure_task .pageBreakBefore }
...
- `MATCHES()` — эта функция принимает строку с темой письма в атрибуте `$ResponseEmailSubject` и регулярное выражение `._Отпуск согласован\. Заявление № \[[0-9]+\]._`, ищет в строке регулярное выражение и возвращает `true`, если выражение найдено. В противном случае функция возвращает `false`.
- `$` — ссылка на текущую запись в шаблоне _«Решения руководителя»_.
- `ResponseEmailSubject` — атрибут _«Тема письма с ответом»_.
- Данная формула возвращает `true`, если тема письма содержит строку «`Отпуск согласован. Заявление № [X]`», где `X` — номер заявления, состоящий из произвольного количества цифр. В противном случае формула возвращает `false`.
...
## Настройка подключений {: #process_email_configure_connections .pageBreakBefore }
...
## Настройка путей передачи данных {: #process_email_configure_routes .pageBreakBefore }
...
- `FORMAT()` — эта функция принимает строку с заполнителями
...
## Тестирование процесса «Согласование отпуска» {: #process_email_configure_test .pageBreakBefore }
...
10. Нажмите кнопку _«Согласовать отпуск»_ в письме.
    {% include-markdown ".snippets/pdfPageBreakHard.md" %}
