...
Имеется шаблон процесса _«Обработка заявок»_ со связанным шаблоном записей _«Заявки»_.
...
- В шаблоне записей _«Заявки»_ [создадим атрибуты для контроля просрочки](#example_n3_task_list_custom_attributes):
...
- В шаблоне процесса _«Обработка заявок»_ [создадим **таблицу задач**](#example_n3_task_list_custom_table) с **системным фильтром** для выборки всех задач всех пользователей и требуемыми столбцами.
...
1. В шаблоне записи _«Заявки»_ создайте атрибут для вычисления признака просрочки:
...
2. В шаблоне _«Заявки»_ создайте атрибут для отображения признака просрочки:
...
3. В шаблоне записи _«Заявки»_ создайте атрибут для вычисления длительности просрочки:
...
1. В шаблоне _«Обработка заявок»_ создайте **таблицу задач** _«Все задачи»_. См. _«[Создание таблицы со списком задач][table_configure_tasks_view]»_.
...
3. Добавьте в столбцы таблицы задач системные атрибуты:
...
4. Добавьте в столбцы таблицы задач атрибуты, показывающие статус просрочки:

    - _Просрочена_
    - _Длительность просрочки_

5. Добавьте таблицу _«Все задачи»_ на требуемую информационную страницу приложения. Например, см. _«[Добавление таблицы на страницу «Мои задачи»][table_configure_add_to_my_tasks]»_.
6. Откройте результирующую таблицу и проверьте её состав.
