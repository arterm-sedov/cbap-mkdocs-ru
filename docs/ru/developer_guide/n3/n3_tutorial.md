...
{
    ("Issue" "IssueTYpe") object:findAttribute ?issueTypeAttribute.
    ("Type" "Title") object:findAttribute ?titleAttribute.
...
**Библиотеки (префиксы)** — это подключаемые модули (области), где лежат знания об объектах, функциях и разных правилах. С их помощью мы можем обращаться с системным и прикладным атрибутам и брать их значения. Также, при объявлении библиотек может появляться доступ к различным функциям по работе с датами, списками, числами и т.д. См. [ссылка на документ][n3_guide]).

С помощью этой библиотеки у нас появляется доступ к функции `object:findAttribute` из строки 3 и 4 нашего примера. Данная функция позволяет брать URI атрибутов, записывать в переменные и использовать их внутри запроса.
...
Префикс `http://comindware.com/ontology/object#` и функция `object:findAttribute` являются базовыми и в дальнейшем будет часто использоваться в написании запросов. Она содержит в себе функцию поиска id атрибута по его системному имени.
...
``` turtle
("TemplateSystemName" "AttributeSystemName") object:findAttribute ?variable.
```
...
{
    ("TemplateOne" "Collection") object:findAttribute ?collectionAttribute.
    ("TemplateTwo" "Number") object:findAttribute ?numberAttribute.
...
В переменную `coll` мы получили коллекцию.  Енумератор взял первую запись из коллекции и пошел на 10 строчку. В этой строке мы делаем точное совпадение (3 тип присваивания). Мы проверяем что у этой записи в атрибуте Number значение 15.
