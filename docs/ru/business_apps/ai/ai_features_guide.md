...
!!! question "Определения"
    ...
    - Для агентов используется «**Системный чат с ИИ-ассистентом**». Этот чат позволяет выстроить взаимодействие между пользователем и **{{ productName }}** в форме диалога.

    - Чаты, создаваемые пользователями, не взаимодействуют со сценариями и **{{ productName }}**. В них можно обсуждать произвольные темы, как в обычном чате с ИИ.

    - Чаты с ИИ располагаются в панели навигации, их можно перемещать, удалять и переименовывать аналогично обычным разделам навигации. См. _«[Использование панели навигации][navigation_panel]»_.
...
3. Добавьте и настройте действие «**Изменить значения переменных**», чтобы сформировать сообщение для LLM.
    ...
    !!! warning "Логика формирования промпта для LLM"
        ...
        3. Функция `LIST($$userMessage->Title)` подставляет исходное сообщение пользователя в промпт для контекста.
        4. Готовый промпт сохраняется в переменной `Message` для передачи в LLM.

    - Введите наглядное имя **набора переменных**, например `prompt`.
    - Создайте переменную с проммтом для LLM:
...
1. Добавьте и настройте действие «**Отправить сообщение**», чтобы передать сформированный промпт в LLM:
...
2. Добавьте и настройте действие «**Выполнить по условиям**» для обработки ответа LLM.
...
3. Внутри действия «**Выполнить по условиям**» в блоке каждого условия добавьте и настройте действие «**Изменить значения переменных**», чтобы сформировать операции для отправки в чат:

    - Введите наглядное имя **набора переменных**, например `actions1`. Набор переменных будет содержать массив операций.
    - Создайте одну или несколько переменных `operation` — это зарезервированное системное имя переменной с JSON-структурой данных операции для чата.
    - Задайте JSON-структуру для каждой операции в столбце «**Значение**» с помощью **формулы**, например:

    ``` json title="Пример JSON-структуры для операции навигации на форму новой записи"
    '{
    ...
    - `objectId` — ID записи, которой требуется перейти, или ID несуществующей записи для создания новой записи (например, `cmw.temp.1`);
    - `template` — ID целевого шаблона записи;
    - `form` — id формы; аналогично можно получить через тройки.
    - `taskId` — служебное поле; не используется, на поведение не влияет.

    !!! question "Массив операций"

        Остальные операции сформируйте аналогичным образом, добавляя их в тот же набор переменных, но с другими идентификаторами записи, формы, шаблона.

    !!! warning "Порядок и отображение операций"

        Порядок операций важен: первая операция всегда выполняется автоматически, а кнопка для неё остаётся в чате для повторного использования.
        
        Кнопки всех операций исчезают из чата после обновления страницы.

    !!! danger "Формирование динамических операций"
        ...
        Экранирование фигурных скобок в `FORMAT()` требуется, потому что одинарные заменители для подстановки данных заключены в одинарные фигурные скобки, например: `{0}`.

        Если фигурные скобки JSON не за экранировать, операция в чате не будет работать.

        Экранирование JSON требуется только в действии «**Изменить значения переменных**», формирующем операции для чата.

        При этом в свойствах пути передачи данных **MockNavigation**, **MessageToNavigation**, а также в действии «**Изменить значения переменных**» экранировать фигурные скобки не требуется: используйте обычный JSON без экранирования (одиночные `{` и `}`).
...
4. Внутри действия «**Выполнить по условиям**» в блоке каждого условия добавьте и настройте действие «**Оправить сообщение в чат**», чтобы сформировать ответ пользователю:
...
Пошаговые инструкции см. выше в разделах [Компиляция адаптера для LLM](#ai_feature_guide_adapter_compile), [Получение ключа для работы с API](#ai_feature_guide_adapter_api_key), [Построение ИИ-агента](#ai_feature_guide_agents) и [Тестирование операций (mock)](#ai_feature_guide_debug_testing).
...
Поле `taskId` зарезервировано и не используется.
