...
### Включение режима отладки {: #ai_feature_guide_debug_testing_enable }
...
## Примеры JSON-структур для отладки и использования в сценариях {: #ai_feature_guide_json_examples .pageBreakBefore }

Ниже даны примеры JSON для настройки полей **MockNavigation** и **MessageToNavigation** для режима отладки.
...
!!! warning "Используйте фактические идентификаторы"
    ...
    Значения идентификаторов в примерах (`event.*`, `oa.*`, `form.*`, `lst.*`, `ds.*`, `fw.*`, `dwc.*` и т.&nbsplд.) приведены для наглядности.
...
### Открыть конкретную запись в определённой форме {: #ai_feature_guide_op_open_instance }
...
### Нажать кнопку за пользователя {: #ai_feature_guide_op_open_instance }
...
### Открыть и отфильтровать определённую таблицу {: #ai_feature_guide_op_open_list }
...
### Актуализация данных с помощью нажатия кнопки в шаблоне {: #ai_feature_guide_op_actualize }
...
### Создание записи {: #ai_feature_guide_op_create_record }
...
### Открытие задачи {: #ai_feature_guide_op_open_task }
...
### Поиск записей {: #ai_feature_guide_full_json_search }
...
### Переход к детализации диаграммы на дашборде {: #ai_feature_guide_op_dashboard }
...
## Открытие формы и переход к определённому полю {: #ai_feature_guide_full_json_navigate_widget }
