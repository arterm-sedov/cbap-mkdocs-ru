...
!!! question "Определения"
    ...
    Чат-боты представляют собой low-code-агенты, построенные на основе **[сценариев][scenarios]**.

    Для low-code-агентов используется «**Системный чат с ИИ-ассистентом**». Этот чат позволяет выстроить взаимодействие между пользователем и **{{ productName }}** в форме диалога.

    Чаты, создаваемые пользователем не взаимодействуют со сценариями, low-code-агентами и **{{ productName }}**. В них можно обсуждать произвольные темы, как в обычном чате с ИИ.

    Чаты с ИИ располагаются в панели навигации, их можно перемещать, удалять и переименовывать.
...
- [Построение low-code-агентов](#ai_feature_guide_agents) — настройка сценариев для обработки сообщений из чата с помощью LLM.
- [Перечень поддерживаемых операций](#ai_feature_guide_operations_list) — справочник по операциям, которые можно выполнять в чате.
- [Примеры JSON для операций](#ai_feature_guide_full_json) — готовые JSON-схемы операций для различных бизнес-задач.
- [Отладка операций](#ai_feature_guide_debug_testing) — проверка выполнения операций в чате без взаимодействия с LLM.
...
5. [Настроить сценарий для работы с чатом](#ai_feature_guide_agents).
...
## Получение ключа для работы с API {: #ai_feature_guide_adapter_api_key .pageBreakBefore }

Для подключения языковой модели необходимо получить ключ API.
...
6. Не устанавливайте флажок «**Использовать режим отладки**» (**ResponseWithMocks**) и не заполняйте поля «**Готовые операции**» (**MockNavigation**) и «**Готовые ответы**» (**MessageToNavigation**). Эти поля используются для тестирования взаимодействия сценариев с чатом.

    !!! tip "Отладка чата без подключения к LLM"

        Чтобы проверить работу чата без взаимодействия с LLM, включите режим отладки, установив флажок «**Использовать готовые ответы**» (**ResponseWithMocks**).
        
        Режим отладки работает следующим образом:
        
        - активен только системный чат, все остальные чаты отключаются;
        - системный чат ожидает ввода определённых ключевых фраз;
        - при вводе в чат ключевой фразы, в чат передаётся соответствующий готовый ответы для имитации поведения LLM.

        Подробные инструкции см. в параграфе _«[Использование режима отладки](#ai_feature_guide_debug_enable)_.

## Построение low-code-агентов {: #ai_feature_guide_agents .pageBreakBefore }

Low-code-агент в **{{ productName }}** строится на основе **сценария** и работает следующим образом:

- сообщение из чата поступает в сценарий посредством события «**Получение сообщения из чата**»;
- сценарий формирует промпт для обработки сообщения с помощью LLM посредством действия «**Изменить значения переменных**»;
- сценарий отправляет сформированный промпт в LLM посредством действия «**Отправить сообщение**»;
- сценарий классифицирует ответ LLM посредством действия «**Выполнить по условиям**»;
- сценарий формирует ответ и набор операций для отправки в чат;
- сценарий отправляет в чат пользователю сообщение с ответом и операциями посредством действия «**Отправить сообщение в чат**»;
- первая операция из набора выполняется от имени пользователя, например заполняется текущая форма;
- пользователь может выполнить остальные полученные операции, нажав соответствующие кнопки в чате.
...
Для использования сценариев необходимы следующие условия:

- настроен адаптер для LLM;
- настроено подключение к LLM;
- Настроен путь передачи данных в LLM согласно требованиям провайдера (например, GigaChat).
...
2. Настройте автоматически созданное событие «**Нажата кнопка**» на получение сообщения из чата:
    ...
    Структура переменной `userMessage`:
...
3. Добавьте и настройте действие «**Изменить значения переменных**», чтобы сформировать сообщение для LLM:
    ...
    - Укажите наглядное имя  **набора переменных**, например `prompt`.
    - Создайте переменную `Message` — это зарезервированное системное имя. Оно должно совпадать с именами атрибутов сообщений в таблицах «**Запрос**» и «**Ответ**» пути передачи данных. См. «[Настройка пути передачи данных для LLM](#ai_feature_guide_route_configure).
    - В **значение** переменной поместите динамически сформированный промпт (инструкции) для LLM, например, с помощью **формулы**:
    ...
    - `LIST($$chat_message->Title)` преобразует сообщение из чата в список для подстановки в функцию `FORMAT()`;
    - `FORMAT()` формирует инструкции (промпт) для LLM и добавляет в него исходное сообщение из чата.

    _![Формирование промпта для LLM в сценарии](img/ai_features_guide_scenario_action_prompt_format.png)_
...
4. Добавьте и настройте действие «**Отправить сообщение**», чтобы передать сформированный промпт в LLM:
    ...
    _![Настройка отправки промпта в LLM](img/ai_features_guide_scenario_action_send_to_llm.png)_
...
5. Добавьте и настройте действие «**Выполнить по условиям**» для обработки ответа LLM:
    ...
    _![Настройка условной обработки ответа LLM](img/ai_features_guide_scenario_action_condition.png)_

6. Внутри действия «**Выполнить по условиям**» в каждом потоке операций добавьте и настройте действие «**Изменить значения переменных**», чтобы сформировать ответ пользователю и операции для отправки в чат:
...
7. Отправка ответа агента в чат пользователю {: #ai_feature_guide_send_to_chat }
    ...
    ### Формирование динамических операций {: #ai_feature_guide_dynamic_operations }
    ...
    !!! danger "Экранирование фигурных скобок в FORMAT()"

        При использовании `FORMAT()` фигурные скобки в JSON необходимо экранировать, записывая их двумя фигурными скобками: .
        
        В противном случае операция не будет работать.

        При этом в полях пути передачи данных (MockNavigation, MessageToNavigation) и в действии «**Изменить значения переменных**» экранировать фигурные скобки не требуется: используйте обычный JSON без экранирования (одиночные `{` и `}`).

## Тестирование операций (mock) {: #ai_feature_guide_debug_testing .pageBreakBefore }

В пути передачи данных есть раздел для тестирования операций без обращения к LLM.

## Перечень поддержанных операций и их составление {: #ai_feature_guide_operations_list .pageBreakBefore }
...
### 1. Открыть экземпляр шаблона записи (форма) {: #ai_feature_guide_op_open_instance }
...
### 2. Открыть настроенный список {: #ai_feature_guide_op_open_list }
...
### 3. Актуализация данных (пользовательская операция) {: #ai_feature_guide_op_actualize }
...
### 4. Создание записи {: #ai_feature_guide_op_create_record }
...
### 5. Открытие задачи {: #ai_feature_guide_op_open_task }
...
### 6. Поиск записей {: #ai_feature_guide_op_search }
...
### 7. Переход в детализацию дашбордов {: #ai_feature_guide_op_dashboard }
...
### 8. Навигация на виджет (конкретное поле) {: #ai_feature_guide_op_navigate_widget }
...
### Примеры ключевых фраз и ответов {: #ai_feature_guide_operations_examples }
...
## Полные примеры JSON по операциям {: #ai_feature_guide_full_json .pageBreakBefore }
...
Пошаговые инструкции см. выше в разделах [Настройка адаптера и операций](#ai_feature_guide_adapter_setup), [Настройка low-code-агентов](#ai_feature_guide_agents) и [Тестирование операций (mock)](#ai_feature_guide_debug_testing).

### Операция 1: открыть экземпляр шаблона записи (форма) {: #ai_feature_guide_full_json_open_instance }
...
### Операция 2: открыть настроенный список {: #ai_feature_guide_full_json_open_list }
...
### Операция 3: актуализация данных (пользовательская операция) {: #ai_feature_guide_full_json_actualize }
...
### Операция 4: создание записи {: #ai_feature_guide_full_json_create_record }
...
### Операция 5: открытие задачи {: #ai_feature_guide_full_json_open_task }
...
### Операция 6: поиск записей {: #ai_feature_guide_full_json_search }
...
### Операция 7: переход в детализацию дашбордов {: #ai_feature_guide_full_json_dashboard }
...
### Операция 8: навигация на виджет (конкретное поле) {: #ai_feature_guide_full_json_navigate_widget }
...
``` json
{
  "Найди мне текстовое поле": {
    ...
    "responseText": "Перешёл на текстовое поле"
  }
}
```

### Использование режима отладки {: #ai_feature_guide_debug_enable }
...
### Поле MockNavigation {: #ai_feature_guide_debug_navigation }
...
### Поле MessageToNavigation {: #ai_feature_guide_message_to_navigation }
...
### Результат вызова {: #ai_feature_guide_debug_result }
...
## Ограничения и планы развития {: #ai_feature_guide_limits_and_roadmap }

Здесь представлена только реализованная на текущий момент функциональность.

Ограничения и запланированные изменения:

- **Обработка сообщений:** обрабатываются сообщения только из системного чата. Выбор источника сообщений (системный чат или чат сценария) в текущей версии не реализован.
