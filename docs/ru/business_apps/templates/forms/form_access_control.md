...
!!! question "Определения"
    ...
    - **Внешняя форма** — форма, на которую ведёт специальная ссылка для внешних пользователей, у которых нет аккаунта в **{{ productName }}**.
...
## Создание внешней ссылки на форму {: #form_access_control_external_form_link }

- Когда внешний пользователь переходит по внешней ссылке, для него создаётся аккаунт в группе «**Анонимные пользователи**».
- Внешние ссылки работают только в мобильном приложении и мобильном браузере.
- В ссылке можно закодировать заранее заполненные данные формы.
- Для формирования и расшифровки внешних ссылок используется метод API `EncryptedNavigationReference`.
