...
Методология описывает общий процесс управления версиями, ветками, средами и релизами в {{ gitVariants }}. Данные принципы и подходы также применимы при управлении версиями с помощью файлов `CTF` и Excel.
...
!!! tip "Описание версий — сообщения коммитов"
...
### Слияние веток для формирования релиза {: #version_control_methodology_transfer_merge_release }
...
1. Убедитесь, что для продуктивной среды настроено и выполняется регулярное резервное копирование, и перед обновлением создана наиболее актуальная резервная копия. См. _«[Резервное копирование и восстановление. Рекомендации по организации и оптимизации][backup_recommendations]»_.
2. Сформируйте новую версию приложения в среде разработки.
3. Выполните шаги, указанные в статье _«[Подготовка приложения к экспорту][version_control_app_prepare]»_.
...
При использовании {{ gitVariants }} откат версии конфигурации в репозитории выполняется на стороне системы контроля версий, а в **{{ productName }}** импортируется соответствующая версия приложения. При использовании файла `CTF` или Excel, выполняется импорт требуемой версии.

!!! warning "Не выполняйте откат в обход системы контроля версий"
...
## Особенности и ограничения импорта и экспорта версий приложения {: #version_control_methodology_transfer_behavior }

Общие особенности поведения при экспорте и импорте, а также перечень сущностей, которые переносятся или не переносятся, подробно описаны в статье _«[Управление версиями приложения. Экспорт и импорт][version_control]»_.
...
Подробную информацию о влиянии экспорта и импорта на идентификаторы и системные имена см. в статье _«[Идентификаторы и системные имена][identifiers_system_names]»_.

## Типовые ошибки и рекомендации по их предотвращению {: #version_control_methodology_common_errors }
...
1. Создайте **среду разработки**, максимально соответствующую текущему состоянию **продуктивной среды**:
   ...
   - разверните копию на отдельном сервере или в отдельном экземпляре, руководствуясь _[рекомендациями по резервному копированию и восстановлению][backup_recommendations]_.

2. Настройте подключение к репозиторию {{ gitVariants }}, если планируется использование Git. См. статью _«[Управление версиями через Git. Настройка подключения][git_connection]»_.
