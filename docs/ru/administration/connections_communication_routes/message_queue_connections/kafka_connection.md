...
## Введение {: #kafka_connection_intro }
...
Если требуется импортировать данные в шаблон записи с сопоставлением полей, поиском записей по ключевым атрибутам и созданием или обновлением записей, используйте _«[Адаптер импорта данных в платформу. Настройка подключения, пути передачи данных и сценария][adapter_data_import]»_. Этот адаптер преобразует входящее XML- или JSON-сообщение во внутреннюю структуру DataImport и запускает сценарий обработки сообщения.

Если требуется отправлять данные из **{{ productName }}** в топик {{ apacheKafkaVariants }}, используйте _«[Адаптер экспорта данных из платформы. Настройка подключения, пути передачи данных и сценария][adapter_data_export]»_.
...
## Прикладная задача {: #kafka_connection_use_case }
...
## Исходные данные {: #kafka_connection_source_data }

- Имеется приложение _«Обработка заказов»_.
...
4. Настройте параметры на вкладке «**Атрибуты сообщений**»:
    ...
    - **Ответ** и **Ответ с ошибкой** — аналогично запросу, сформируйте структуру атрибутов для отправки серверу {{ apacheKafkaVariants }} ответа на полученный запрос. Значения этим атрибутам можно присвоить с помощью [сценария](#kafka_connection_scenario), срабатывающего при наступлении события «**Получение сообщения**».
...
## Настройка сценария {: #kafka_connection_scenario .pageBreakBefore }
...
## Создание топика и тестирование работы интеграции {: #kafka_connection_test .pageBreakBefore }
...
## Просмотр журнала работы сценария {: #kafka_connection_scenario_log }
...
- _[Адаптер импорта данных в платформу. Настройка подключения, пути передачи данных и сценария][adapter_data_import]_
