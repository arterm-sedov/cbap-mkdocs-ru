...
Если внешняя система передаёт данные не через брокер сообщений, а прямым HTTP-запросом, используйте _«[Адаптер для импорта данных. Настройка подключения, пути передачи данных и сценария][adapter_data_import]»_. Этот адаптер принимает HTTP-запрос, преобразует XML-сообщение во внутреннюю структуру DataImport и запускает сценарий обработки сообщения.
