...
- в информационной панели при нажатии значка «**Уведомления**» <i class="fa-light fa-bell ">‌</i> (сервер должен быть подключен к {{ apacheKafkaVariants }});
- в push-сообщениях при использовании **[{{ productNameMobile }}][mobile_app_use]** и веб-версии **{{ productName }}** (сервер должен быть подключен к интернету и {{ apacheKafkaVariants }});
