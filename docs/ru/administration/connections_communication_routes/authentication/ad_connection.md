...
- **Домен** — укажите домен сервера каталогов, используемый для аутентификации.
    ...
    - В поле **«Домен»** укажите домен, который будет использоваться как **домен по умолчанию** для данного подключения и (при настройке SSO через Kerberos в Linux) как домен по умолчанию для аутентификации. См. _«[SSO-аутентификация. Настройка ПО, контроллера домена и ПК пользователя][sso_authentication_configure]»_.
    ...
    - Пользователи других доменов могут проходить аутентификацию, указывая домен явно, например `NPS\user1` или `user1@NPS.COMPANY.LOCAL`.
