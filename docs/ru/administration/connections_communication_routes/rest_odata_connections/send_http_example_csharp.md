...
- Имеется шаблон _«Заказы»_ с атрибутами:
...
- В шаблоне _«Позиции заказов»_ имеются атрибуты _«Товар»_ типа «**Текст**» и _«Количество»_ типа «**Число**».
...
- **Запрос** — добавьте следующие атрибуты:
    - `orderNumber` (системное имя должно совпадать с именем поля номера заказа на внешнем сервере) типа «**Строка**».
    - `items` (системное имя должно совпадать с именем поля позиций на внешнем сервере) типа «**Объект**».
        - Установите флажок «**Массив**».
        - Установите флажок у атрибута `items` и добавьте в него дочерние атрибуты:
            - `product` (системное имя должно совпадать с именем поля товара на внешнем сервере) типа «**Строка**» — этот атрибут будет содержать название товара.
            - `number` (системное имя должно совпадать с именем поля количества на внешнем сервере) типа «**Число**» — этот атрибут будет содержать количество товара.
...
- **Шаблон пути запроса** — введите `place-order` (фактический путь к HTTP-запросу на сервере), этот суффикс будет добавлен к **URI**, указанному в [свойствах подключения для отправки HTTP-запросов][http_send_connection]. Например, для **URI** `https://<hostname>/api` результирующий путь запроса будет `https://<hostname>/api/items`.
...
3. После действия действия «**Нажатие кнопки**» добавьте действие «**Проверить результат скрипта**» для подготовки данных HTTP-запроса с помощью C#-скрипта следующим образом:
    ...
    class Script
    {
        ...
        {      
            ...
            foreach(var itemId in itemsIdsArray)
            {
                ...
                var item = new Item{product = itemData[itemId.ToString()]["Количество"].ToString() ,number = Convert.ToInt32(itemData[itemId.ToString()]["Количество"].ToString())};
    ...
    **Образец скрипта для проверки ответа:**
    ...
    class Script
    {
        public static bool Main(Object o)
        {
            ...
            {
                Api.TeamNetwork.ObjectService.EditWithAlias(id, new Dictionary<string,object>{{"СтатусЗаказа",inputSuccess.orderStatus}});
            }
            else if(inputError != null)
            {
                Api.TeamNetwork.ObjectService.EditWithAlias(id, new Dictionary<string,object>{{"СтатусЗаказа",inputError.orderStatus}});
            }
            else
            {
