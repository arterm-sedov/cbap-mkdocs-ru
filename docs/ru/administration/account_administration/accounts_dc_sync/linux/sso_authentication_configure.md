...
!!! note "Работа с несколькими доменами"
    ...
    Здесь представлены настройки для одного домена Active Directory, указанного как **домен по умолчанию** (default realm) в конфигурации Kerberos (`default_realm` в `krb5.conf`) и в параметрах NGINX (`auth_gss_realm`). 
    ...
    - SSO одновременно из нескольких доменов можно реализовать во внешней инфраструктуре (например, через федерацию удостоверений на базе Keycloak или AD FS) с дальнейшей аутентификацией в **{{ productName }}** по протоколу OpenID Connect, см. _«[Аутентификация через Keycloak и OpenID Connect. Настройка подключения и служб][openid_connection]»_.
...
!!! warning "Передача идентификатора домена (realm) браузером"
