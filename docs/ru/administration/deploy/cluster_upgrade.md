...
- **PLATFORM-node0** (`XX.XX.101.10`) — основной узел пользовательского трафика. 80% — пользовательский трафик, 20% — интеграционный.
- **PLATFORM-node1** (`XX.XX.101.11`) — узел мониторинга и поиска. 60% — пользовательский трафик, 40% — интеграционный.
- **PLATFORM-node2** (`XX.XX.101.12`) — узел поиска и обработки данных. 40% — пользовательский трафик, 60% — интеграционный.
- **PLATFORM-node3** (`XX.XX.101.13`) — основной узел трафика интеграций. . 80% — интеграционный трафик, 20% — пользовательский.

!!! warning "Распределение трафика"

    При ином количестве узлов, а также при исключении узлов на время обновления балансировщик нагрузки должен распределять трафик пропорционально с учётом ваших бизнес-потребностей, производительности узлов и характера трафика.
...
Балансировщик нагрузки (`XX.XX.106.43`) рекомендуется настроить следующим образом:

- **Балансировщик** должен распределять трафик согласно бизнес-потребностям и требованиям доступности:
    - **Пользовательский трафик**: `PLATFORM-node0` (40%), `PLATFORM-node1` (30%), `PLATFORM-node2` (20%).
    - **Трафик CRM-системы**: распределяется между `PLATFORM-node0`, `PLATFORM-node1`, `PLATFORM-node2`.
    - **Остальные узлы**: PLATFORM-node3 и `PLATFORM-node5` обрабатывают дополнительную нагрузку
- **Контроль исправности узлов:**
    - Все узлы предоставляют эндпоинт `api/health` для контроля исправности.
    - `api/health` возвращает статус `200`, если узел **{{ productName }}** работает штатным образом.
    - Балансировщик должен автоматически исключать неисправные узлы.

### Инфраструктура данных {: #cluster_upgrade_data_infrastructure }

- **Распределённая файловая система**
    - **DFS** (`//dfs-01/company.ru/sdapps$`) — общее хранилище для всех узлов.
    - Обеспечивает синхронизацию данных между всеми узлами.
- **Кластер поиска**
    - **{{ openSearchVariants }}** (`XX.XX.101.1137-139`) — кластер поиска для узлов 1–4.
    - Обеспечивает индексирование и поиск данных.
- **Распределённое хранилище данных**
    - **{{ apacheIgniteVariants }}** — распределённое хранилище, которое реплицирует данные между узлами.
...
1. Уберите узел `PLATFORM-node0` из списка узлов для балансировки нагрузки.
2. Проверьте корректность ребалансировки нагрузки посредством инструментов мониторинга:
...
### Этап 3. Восстановление пользовательского трафика {: #cluster_upgrade_step3 }

Снова направим пользовательский трафик на обновлённый узел `PLATFORM-node0`.

**Цель**: вернуть обновлённый узел в эксплуатацию в составе кластера.

1. С помощью балансировщика нагрузки восстановите передачу трафика на обновлённый `PLATFORM-node0`.
2. Проверьте работоспособность **{{ productName }}** с включённым узлом `PLATFORM-node0` посредством инструментов мониторинга:

    - Убедитесь, что пользовательский трафик поступает на `PLATFORM-node0` в заданной на балансировщике нагрузки пропорции.
    - Убедитесь, что пользовательские функции работают корректно.
    - Проверьте время отклика системы.
    - Проконтролируйте ошибки посредством журналов.

### Этап 4. Восстановление интеграционного трафика {: #cluster_upgrade_step4 }

Снова направим интеграционный трафик на обновлённый узел `PLATFORM-node0` в требуемой пропорции.

**Цель**: вернуть интеграционный трафик на обновлённый узел.

1. С помощью балансировщика нагрузки восстановите передачу интеграционных запросов на `PLATFORM-node0`.
2. Проверьте работоспособность интеграций с включённым узлом `PLATFORM-node0` посредством инструментов мониторинга:

    - Убедитесь, что интеграционный трафик поступает на `PLATFORM-node0` в заданной на балансировщике нагрузки пропорции.
    - Убедитесь, что эндпоинт `api/health` на `PLATFORM-node0` выдаёт статус `200`.
    - Проверьте соединение узла `PLATFORM-node0` с кластером {{ apacheIgniteVariants }}.
    - Проверьте обработку входящих и исходящих интеграционных запросов.
    - Проконтролируйте производительность интеграций.
    - Проанализируйте журналы на предмет наличия ошибок.

### Этап 5. Обновление остальных узлов {: #cluster_upgrade_step5 }
