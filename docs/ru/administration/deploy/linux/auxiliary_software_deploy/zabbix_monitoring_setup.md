...
# Настройка мониторинга с использованием Zabbix {: #zabbix_monitoring_setup }
...
## Порядок развёртывания {: #zabbix_monitoring_setup_sequence }
...
2. **Получение и импорт пакета мониторинга**:
    ...
    - Запросите пакет для развёртывания мониторинга в компании **{{ companyName }}**.
...
## Состав пакета мониторинга {: #zabbix_monitoring_setup_package_contents }

Чтобы развёрнуть Zabbix для мониторинга **{{ productName }}**, запросите пакет для развёртывания мониторинга в компании **{{ companyName }}**.
...
| `comindware-host-installer/`                                          | Установщик UserParameter, скриптов, stub_status nginx, автоматическая установка `python3`/`gawk` при необходимости, настройка группы `adm` для чтения логов nginx |
...
## Предварительные условия {: #zabbix_monitoring_setup_prerequisites }
...
- **Серверная инфраструктура:**
    ...
    - На целевых серверах установлены:
      ...
      - **{{ nginxVariants }}** — веб-сервер (требуется для метрик из журнала доступа и `stub_status`).
...
- **Журналирование:**
    - Для метрик **журнала {{ nginxVariants }}** имеется файл `/var/log/nginx/<INSTANCENAME>-access.log`.
    - Журнал хранится в формате JSON-подобных строк с полями `time`, `status`, `duration` (типовой `log_format` **{{ productName }}**).

## Импорт шаблона в Zabbix {: #zabbix_monitoring_setup_template_import }
...
## Создание хоста и привязка шаблона {: #zabbix_monitoring_setup_host_configuration }
...
## Настройка макросов на уровне хоста {: #zabbix_monitoring_setup_macros }

### Обязательные макросы {: #zabbix_monitoring_setup_macros_required }
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `{$INSTANCENAME}` | Имя экземпляра **{{ productName }}** (используется в путях журналов, {{ apacheIgniteVariants }}, юнитах `systemd`)                                                           | `<instanceName>`             |
...
### Рекомендуемые макросы {: #zabbix_monitoring_setup_macros_recommended }
...
## Установка компонентов на хосте {: #zabbix_monitoring_setup_host_installation }

### Подготовка к установке {: #zabbix_monitoring_setup_host_installation }

Скопируйте на целевой сервер каталог `release/comindware-host-installer/` (например, в `/root/comindware-host-installer/`).

### Запуск установщика {: #zabbix_monitoring_setup_host_installation_start }
...
2. Установщик автоматически выполнит следующие операции:
    ...
    - **Настройка {{ nginxVariants }}** — включение `stub_status`:
...
### Зависимости скриптов {: #zabbix_monitoring_setup_host_installation_dependencies }

- **Метрики журнала {{ nginxVariants }}:**
    - основной парсер: **Python 3** + скрипт `nginx_access_log_metrics.py`
    - резервный вариант: **gawk** (при отсутствии Python)
- **Метрики {{ apacheIgniteVariants }}:**
...
### Настройка прав доступа к логам nginx {: #zabbix_monitoring_setup_host_installation_access }
...
### Нормализация окончаний строк между Linux и Windows {: #zabbix_monitoring_setup_host_installation_normalization }
...
## Проверка работоспособности {: #zabbix_monitoring_setup_verification }

### Проверка на хосте {: #zabbix_monitoring_setup_verification_host }

Выполните тестовые запросы к агенту (подставьте имя вашего инстанса вместо `<INSTANCENAME>`):

``` bash
# Проверка метрик {{ apacheIgniteVariants }}
sudo zabbix_agent2 -t 'ignite.metrics[cpu_cur_load,<INSTANCENAME>]'

# Проверка метрик журнала доступа {{ nginxVariants }}
sudo zabbix_agent2 -t 'nginx.access.status_200[<INSTANCENAME>,300]'

# Проверка статуса службы {{ companyName }}
sudo zabbix_agent2 -t 'service.status[comindware<INSTANCENAME>.service]'

# Проверка stub_status {{ nginxVariants }}
sudo zabbix_agent2 -t 'web.page.get["127.0.0.1","nginx_status","81"]'
```

### Проверка в веб-интерфейсе Zabbix {: #zabbix_monitoring_setup_verification_zabbix }
...
- [Развёртывание Zabbix Server][zabbix_server_deploy]
- [Развёртывание Zabbix Agent][zabbix_agent_deploy]
- [Общие сведения о развёртывании Zabbix][zabbix_deploy]
