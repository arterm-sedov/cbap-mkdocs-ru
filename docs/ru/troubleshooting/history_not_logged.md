...
- **Подключение из {{ productName }} настроено неверно**: исправьте URL/учётные данные/префикс и нажмите «Проверить соединение». См. и _«[Подключения][connections]»_.
...
1. Комплексная проверка {{ openSearchVariants }} и {{ productName }}
    ...
    - Корректность параметров подключения в {{ productName }}:
        ...
        - Нажмите «Проверить соединение» — должно отобразиться «Соединение установлено».
