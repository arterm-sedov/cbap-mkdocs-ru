---
title: 'История изменений не записывается. Диагностика и восстановление'
kbId: 
tags:
  - Elasticsearch
  - OpenSearch
...
# История изменений не записывается. Диагностика и восстановление {: #history_not_written }
...
- **Подключение из платформы настроено неверно**: исправьте URL/учётные данные/префикс и нажмите «Проверить соединение». См. _«[{{ openSearchVariants }}. Настройка подключения][elasticsearch_connection]»_ и _«[Подключения][connections]»_.
...
1. Комплексная проверка {{ openSearchVariants }} и платформы
    ...
    - Корректность параметров подключения в платформе:
...
6. Если маппинг некорректен/повреждён — выполнить реиндексацию
    ...
    - Перед реиндексацией важно сверить маппинг с эталоном: выполните `GET /<index>/_mapping` и сравните с `m1.json`.
    - Убедитесь, что у вас есть валидный JSON с маппингом (например, `m1.json`).
    - Возьмите префикс индексов из подключения платформы (UI) — далее он будет использован скриптом.




## Реиндексация индексов по префиксу (локальный {{ openSearchVariants }})

Ниже кратко описан скрипт и порядок применения. Скрипт размещён в репозитории документации: `docs/ru/troubleshooting/assets/reindex_local_prefix.sh`.

### Назначение скрипта

- Найти все индексы по шаблону `cmw_<prefix>*`.

ВАЖНО Нас интересуют индексы с суффиксами `_sln.xx....`
...
- `-p | --prefix` — платформенный префикс (например, `support` → ищет индексы `cmw_support*`).
...
!!! warning "Перед выполнением скрипта реиндексации"
    ...
    - Для остановки платформы выберите сервисное окно, когда нагрузка минимальна.
...
``` bash
bash reindex_local_prefix.sh -p <platformPrefix> -ip localhost:9200 -m m1.json
```

- Удалённый сервер:

``` bash
bash reindex_local_prefix.sh -p <platformPrefix> -ip 10.0.0.5:9200 -m m1.json
```

После выполнения реиндексации проверьте результат:

- Убедитесь, что схема полей совпадает с эталоном (`m1.json`), нет лишних/неверных типов.
...
- _[Архитектура и ландшафт развертывания][architecture_landscape]_
- _[Системные требования][system_requirements]_
- _[Пути и содержимое директорий экземпляра ПО][paths]_
- _[Установочные скрипты. Назначение и ключи][script_keys]_
- _[Elasticsearch. Установка в базовой конфигурации для Windows][elasticsearch_deploy_windows]_
- _[Развёртывание Elasticsearch без сертификатов подлинности][elasticsearch_cluster_deploy_no_certificates]_
- _[Elasticsearch. Установка в базовой конфигурации][elasticsearch_deploy_Linux]_
- _[Настройка SSL-сертификатов для Elasticsearch][elasticsearch_ssl_certificate_configure]_
...
- _[Ошибки в процессе. Отслеживание][process_error_monitor]_
