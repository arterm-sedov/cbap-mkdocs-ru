...
## Настройка собственных тестовых сценариев в Apache JMeter {: #load_testing_jmeter_scenarios }

В готовом файле конфигурации `jmeter-example-settings.jmx` представлены примеры следующих сценариев тестирования:

- открыть таблицу;
- сохранить форму с заполненными данными;
- нажатие пользовательской кнопки по выбранным записям из таблицы.

По образцу этих примеров создайте собственные сценарии:

1. В разделе **Test Plan** создайте копию готового сценария: нажмите сценарий правой кнопкой мыши и выберите пункт **Duplicate**.
2. В новом сценарии создайте контроллер транзакций:

    - нажмите сценарий правой кнопкой миши;
...
3. В новом сценарии создайте HTTP-запрос:

    - нажмите требуемое действие правой кнопкой миши;
...
## Получение путей и тела тестового запроса {: #load_testing_jmeter_get_request }

1. Откройте в **{{ productName }}** страницу с формой или таблицей, которую требуется протестировать.
2. В браузере включите режим разработки DevTools, например, нажав клавишу ++F12++ или выбрав в меню настроек пункты «**Дополнительные инструменты**» — «**Инструменты разработчика**».
3. Выберите вкладку **Network**. Чтобы получить данные, нажмите в **{{ productName }}** любую кнопку, перезагрузите страницу, выберите значение в раскрывающемся списке или заполните поле.
4. В таблице **Name** найдите элемент, который требуется протестировать: **Query** (таблица), **QueryForm** (форма), **Execute** (кнопка), **QueryData** (заполнение поля).
5. Путь будет указан на вкладке **Headers** в строке **Request URL** после адреса **{{ productName }}**.

    _![Просмотр пути HTTP-запроса на вкладке Network в браузере](img/browser_network_tab.png)_

6. Тело запроса будет отображаться на вкладке **Payload**.
7. Скопируйте тело запроса: нажмите правой кнопкой мыши первую строку под заголовком **Request Payload** и выберите пункт **Copy value** или **Copy object**.
...
!!! tip "Подстановка ID записей в теле запроса"
    ...
    Поле `objId` в теле запроса — это за ID записи, для которой выполняется запрос.
...
## Выполнение тестовых сценариев под разными пользовательскими ролями
    ...
    Если требуется запускать сценарии одновременно под пользователями с разными ролями:
    
1. Добавьте новую группу пользователей **Thread Group** — сделайте дубликат существующей.
2. В **Test Plan** добавьте новые переменные с данными для авторизации пользователя под конкретной ролью.
3. В разделе **HTTP Authorization Manager**» в полях **Username** и **Password** укажите учётные данные тестового аккаунта в **{{ productName }}**.
