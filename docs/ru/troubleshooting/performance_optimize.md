...
kbTitle: 'Факторы, влияющие на производительность. Рекомендации по оптимизации'
kbId: 4712
tags:
tags:
  - N3
...
## Введение {: #performance_optimize_intro }
...
## Оптимизация времени отклика {: #performance_optimize_response_time }
...
## Оптимизация интеграций с внешними службами {: #performance_optimize_external_services }
...
7. **Службы {{ apacheKafkaVariants }}, {{ apacheIgniteVariants }} и {{ openSearchVariants }}**

    - Оптимизируйте конфигурацию служб {{ apacheKafkaVariants }}, {{ apacheIgniteVariants }} и {{ openSearchVariants }}:
        - выделение памяти;
        - размер кучи Java.
    - Проверьте и настройте конфигурации репликации и шардирования данных для {{ openSearchVariants }}.
    - Настройте параметры устойчивости и производительности {{ apacheIgniteVariants }}, такие как количество узлов и сетевые параметры.
    - Настройте параметры устойчивости и производительности {{ apacheKafkaVariants }}, такие как период хранения сообщений и сегментация журналов.
    - Регулярно дефрагментируйте базу данных {{ apacheIgniteVariants }} для ускорения доступа и освобождения места.

8. **Антивирус**

    - Проверьте, не снижается ли производительность из-за работы антивируса на сервере.
    - Рассмотрите возможность настройки исключений для служб {{ apacheKafkaVariants }}, {{ apacheIgniteVariants }} и {{ openSearchVariants }} в антивирусном программном обеспечении.

9. **Ненужные внешние службы**

    - Отключите ненужные внешние службы для повышения производительности.

## Оптимизация выполнения процессов и токенов {: #performance_optimize_processes_tokens }
...
## Оптимизация публикации процессов и токенов {: #performance_optimize_publishing }

1. **Количество версий процессов**

    - Сократите количество версий диаграмм процессов, оставив только необходимые.

## Мониторинг и диагностика {: #performance_optimize_monitoring }
...
1. **Мониторинг и трассировка**
    ...
    - Используйте инструменты мониторинга:
     - {{ openSearchVariants }} — Kibana.
     - Apache {{ apacheKafkaVariants }} — AKHQ, Kowl, Kafdrop, UI for Apache Kafka, Lenses, CMAK, Confluent CC, Conduktor.
     - {{ apacheIgniteVariants }} — JMX и MBeans, системные представления в SQL, Yardstick Framework.
