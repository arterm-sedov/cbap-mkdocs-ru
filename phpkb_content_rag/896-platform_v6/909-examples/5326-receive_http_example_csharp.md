...
kbId: 5326
url: 'https://kb.comindware.ru/article.php?id=5326'
...
Здесь представлен пример настройки подключения, пути передачи данных и [сценария](https://kb.comindware.ru/article.php?id=5654) для получения и обработки HTTP-запроса в формате JSON с использованием C#-скриптов и записи данных в атрибуты шаблона записи.
...
_![Настройка подключения для получения HTTP-запросов с помощью сценариев](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/receive_http_example_csharp_connection_create.png)_
...
_![Настройка основных свойств пути для получения HTTP-запросов](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/receive_http_example_csharp_route_general.png)_
...
3. В таблице «**Ответ**» составьте структуру атрибутов для ответа в случае успешной обработки заказа:
   ...
   ![Настройка атрибутов сообщения](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/receive_http_example_csharp_route_message_attributes.png)
...
3. При необходимости укажите **атрибут для заголовков**, в котором будут содержаться все атрибуты заголовков запроса, **атрибут для параметров запроса**, в котором будут содержаться все параметры запроса, и **атрибут для тела запроса**, в котором будет содержаться всё тело запроса.
   ...
   ![Настройка интеграции](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/receive_http_example_csharp_route_integration.png)
...
1. Создайте новый [сценарий](https://kb.comindware.ru/article.php?id=5654) *«Обработка заказов»*.
2. У начального события измените тип на «**Получение сообщения**» и настройте его:
   ...
   ![Настройка действия «Получение сообщения»](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/receive_http_example_csharp_scenario_receive_message.png)
...
4. Результирующий сценарий должен выглядеть, как показано на следующей иллюстрации.
   ...
   ![Сценарий для получения HTTP-запроса с использованием C#-скриптов](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/receive_http_example_csharp_scenario.png)
