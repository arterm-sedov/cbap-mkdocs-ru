...
kbId: 5317
url: 'https://kb.comindware.ru/article.php?id=5317'
...
Ограничения некоторых S3‑совместимых сервисов

Некоторые S3‑совместимых сервисов (например, Yandex Object Storage, Amazon S3, MinIO) могут ограничивать размер одиночного запроса `PUT`, как правило, до 5 ГБ. Если объём передаваемых данных превышает лимит и выгрузка выполняется как один `PUT`‑запрос, соединение будет прервано сервисом по причине превышения лимита.

Учитывайте ограничения конкретного провайдера при выборе хранилища для резервных копий и пользовательских файлов.

Если вам необходимо отправлять в S3 файлы больше лимита, установленного вашим поставщиком S3, обратитесь в службу поддержки **Comindware** для настройки конфигурации **Comindware Platform**.
...
- Необходимо указать разные корзины для резервных копий, загруженных и временных файлов.
  - Сведения о назначении имени корзины для резервного копирования см. в статье *«[Резервное копирование. Настройка, запуск и просмотр журнала сеансов](https://kb.comindware.ru/article.php?id=5566)»*.
