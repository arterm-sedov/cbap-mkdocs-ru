...
kbId: 5313
url: 'https://kb.comindware.ru/article.php?id=5313'
updated: '2025-12-25 16:38:15'
---
...
См. также *«[HTTP-запросы. Получение JSON-данных в сценарии](https://kb.comindware.ru/article.php?id=5311)»*
...
1. Настройте [шаблон записи](#http_receive_file_template) для хранения полученных данных.
2. Настройте [подключение](#connection_configure) для получения HTTP-запросов.
3. Настройте [путь передачи данных](#communication_route_configure) для получения HTTP-запросов.
4. Настройте [сценарий](#scenario_configure) для обработки входящих HTTP-запросов.
5. [Проверьте работоспособность](#testing) настроенной конфигурации.
...
3. Настройте подключение к серверу:
   ...
   - **Базовый путь получения HTTP-запросов** — добавьте **путь URI**, например `httpListener`. При необходимости введите дополнительный **путь URI** на [вкладке «**Интеграция**» в свойствах пути передачи данных](#http_receive_file_route_integration). Укажите результирующий путь на внешнем сервере в качестве получателя запроса, например:
...
4. Настройте свойства пути передачи данных на следующих вкладках:
   ...
   - [**Основные свойства**](#http_receive_file_route_general)
   - [**Атрибуты сообщений**](#http_receive_file_route_attributes)
   - [**Интеграция**](#http_receive_file_route_integration)
...
- **Подключение** — выберите [подключение для получения HTTP-запросов](#http_receive_file_connection).
...
1. Укажите дополнительный **путь URI**, например `uploadFiles`.
   ...
   **Путь URI** с вкладки «**Интеграция**» будет добавлен к **базовому пути получения HTTP-запросов** (например, `https://<hostname>/api/public/adapter/httpListener`). Базовый путь настраивается в свойствах [подключения](#http_receive_file_connection).
...
2. Откройте событие «**Нажатие кнопки**» и настройте его следующим образом:
   ...
   - **Подключение:** [подключение для получения документов с помощью HTTP-запросов](#http_receive_file_connection);
   - **Путь передачи данных:** [путь передачи данных для получения документов с помощью HTTP-запросов](#http_receive_file_route);
...
4. Добавьте действие «**Изменить значение атрибутов**» внутри действия «**Создать запись**» со следующими свойствами:
   ...
   - **Значение: формула**
     ...
     Здесь `Description` — имя атрибута для хранения описания файлов, настроенного на [вкладке «**Атрибуты сообщений**» пути передачи данных](#http_receive_file_route_attributes).
5. Добавьте действие «**Повторять по количеству объектов**» внутри действия «**Создать запись**» со следующими свойствами:
   ...
   - **Атрибут или выражение для поиска объектов: формула**
     ...
     Здесь `FileObject` — имя атрибута для хранения полученных файлов, настроенного на [вкладке «**Атрибуты сообщений**» пути передачи данных](#http_receive_file_route_attributes).
...
_![Сценарий получения вложений с помощью HTTP-запросов](/platform/v6.0/administration/connections_communication_routes/rest_odata_connections/img/http_receive_file_scenario.png)_

## Тестирование

1. С внешнего сервера отправьте в **Comindware Platform** HTTP-запрос с несколькими файлами, например:
   ...
   - `https://<hostname>/api/public/adapter/httpListener/uploadFiles` — **базовый путь получения HTTP-запросов** и **путь URI**, настроенные в [пути передачи данных](#http_receive_file_route);
