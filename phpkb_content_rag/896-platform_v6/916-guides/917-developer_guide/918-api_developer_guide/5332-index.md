...
kbId: 5332
url: 'https://kb.comindware.ru/article.php?id=5332'
...
**Comindware Platform** может обращаться к API внешних систем через [подключения](https://kb.comindware.ru/article.php?id=5303) и [пути передачи данных](https://kb.comindware.ru/article.php?id=5302), а также может обрабатывать запросы из внешних систем через интерфейсы **REST API** трёх типов:

- [**Solution API**](https://kb.comindware.ru/article.php?id=5333) — обеспечивает доступ к актуальной модели и бизнес-логике конкретного приложения.
- [**System Core API**](https://kb.comindware.ru/article.php?id=5331) — предоставляет доступ к системным функциям (управление аккаунтами, системными службами и т.д.).
- [**Web API**](https://kb.comindware.ru/article.php?id=5330) — универсальные методы прикладного уровня для работы с данными (излечение записей, базовые CRUD-операции).
...
Перед выбором API ознакомьтесь с параграфом *«[**Обратная совместимость API**](#api_intro_backward_compatibility)»*.
...
- Для системных задач (аутентификация, настройки) используйте **System Core API**. Учитывайте, что при обновлении мажорной версии ПО (например, с 4.2 на 6.0) может потребоваться обновить код интеграций.
...
- **Рекомендации**: при обновлении мажорной версии (например, с 4.x.x на 6.0.0) проверяйте документацию на наличие изменений. Для минорных версий (например, 5.1 и 5.2) совместимость сохраняется.
...
- Запрос `GET` или `DELETE` в виде URL-строки, содержащей все параметры запроса.
  - Внешняя система отправляет запрос в строке URL. Заготовку запроса для внешней системы можно сформировать в интерфейсе [Swagger](#api_intro_swagger).
...
- Запрос `POST` или `PUT` с телом, содержащим все параметры (например, имя пользователя и пароль, данные, которые необходимо отправить или получить и т. д.).
  - Внешняя система отправляет запрос в формате JSON или XML. Заготовку запроса для внешней системы можно сформировать в интерфейсе [Swagger](#api_intro_swagger).
...
- HMAC —  требуется передать токен и секретный ключ, сформированные для аккаунта на странице «**Администрирование**» – «**[Ключи аутентификации](https://kb.comindware.ru/article.php?id=5613)**».

Примечание

- Для использования методов API и интерфейса [Swagger](#api_intro_swagger) следует создать специальный аккаунт и предоставить предоставить ему разрешение «**Вызов API**» в системной роли. См. *«[Системные роли. Определения, настройка, объединение, удаление](https://kb.comindware.ru/article.php?id=5610)»*. Кроме того, API и Swagger могут использовать пользователи, входящие в системную роль «**Системные администраторы**».
- При вызове методов API в интерфейсе [Swagger](#api_intro_swagger) на странице `https://your-host/docs` уже выполнен вход в **Comindware Platform**, и дополнительная аутентификация не требуется.
...
- Запрос отправляет скрипт на C# (вызываемый [кнопкой](https://kb.comindware.ru/article.php?id=5728) или [задачей выполнения сценария](https://kb.comindware.ru/article.php?id=5668) в процессе). Скрипт устанавливает подключение к внешней системе, формирует запрос, отправляет его и получает ответ.
- Запрос формирует и отправляет [сценарий](https://kb.comindware.ru/article.php?id=5654) посредством настроенного подключения к внешней системе. Сценарий позволяет сформировать тело запроса, проанализировать полученный ответ и поместить из него данные в атрибуты записей.
- Запрос формирует и отправляет [промежуточное](https://kb.comindware.ru/article.php?id=5676) или [конечное событие](https://kb.comindware.ru/article.php?id=5680) отправки сообщения в процессе.

Примечание

Для отправки запросов с помощью сценариев и событий процесса требуется настроить [подключение](https://kb.comindware.ru/article.php?id=5303) к внешней службе и [путь передачи данных](https://kb.comindware.ru/article.php?id=5302).

### Аутентификация при доступе к API внешних систем

При доступе к внешним API используйте метод аутентификации, который поддерживает внешняя система. Обратитесь к документации API этой системы, чтобы узнать, следует ли указывать имя пользователя и пароль в URL-адресе или теле запроса, или необходимо сгенерировать и использовать токен безопасности вместо передачи пароля в виде обычного текста.
...
2. Отобразится страница **Comindware Platform API** со следующими разделами:
   ...
   - **RESTful Web API** — общие методы для всех версий ПО (см. [Методы Web API](https://kb.comindware.ru/article.php?id=5330));
   - **System Core API** — системные методы, которые могут отличаться для разных версий ПО (см. [Методы System Core API](https://kb.comindware.ru/article.php?id=5331));
   - **Solution API** — методы для шаблонов бизнес-приложений (см. [Методы Solution API](https://kb.comindware.ru/article.php?id=5333)).
...
_![Страница Swagger UI с разделами API Comindware Platform](https://kb.comindware.ru/assets/img_661d06c21de49.png)_
