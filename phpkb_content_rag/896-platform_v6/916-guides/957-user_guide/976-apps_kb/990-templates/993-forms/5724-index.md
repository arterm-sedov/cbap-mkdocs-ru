...
kbId: 5724
url: 'https://kb.comindware.ru/article.php?id=5724'
updated: '2025-02-28 19:14:33'
---
...
- На форме можно разместить поля, привязанные к атрибутам, вложенные формы, статичный текст, а также различные визуальные элементы. См.:
  - *[Динамические элементы формы: поля атрибутов и вложенные формы. Настройка представления](https://kb.comindware.ru/article.php?id=5723)»*
...
- Инструкции по использованию форм для пользователей см. в статье *«[Формы. Использование](https://kb.comindware.ru/article.php?id=5603)»*.

## Просмотр списка и настройка форм шаблона

Примечание

- Список форм предусмотрен у шаблонов [записей](https://kb.comindware.ru/article.php?id=5693), [аккаунтов](https://kb.comindware.ru/article.php?id=5695), [ролей](https://kb.comindware.ru/article.php?id=5692) и [организационных единиц](https://kb.comindware.ru/article.php?id=5697).
- В [шаблоне процесса](https://kb.comindware.ru/article.php?id=5694) список форм не предусмотрен, и формы создаются и настраиваются для элементов диаграммы процесса. См. статью *«[Стартовая форма и форма пользовательской задачи](https://kb.comindware.ru/article.php?id=5660)»*.
...
_![Список форм шаблона](/platform/v6.0/business_apps/templates/forms/img/form_template_list.png)_
...
_![Конструктор формы](/platform/v6.0/business_apps/templates/forms/img/form_designer.png)_
...
3. Настройте свойства формы:
   ...
   - **Тип**:
     ...
     - **Внутренняя** — выберите этот тип, чтобы форма отображалась, только если она вложена в другую форму или указана в качестве формы для записей (например, при запуске процесса, при создании записи, в диалоговых окнах). См. *«[Публичные и внутренние формы. Контроль доступа](https://kb.comindware.ru/article.php?id=5726)»*.
     - **Форма поиска** — выберите этот тип, чтобы настроить расширенную фильтрацию записей. См. *«[Расширенная фильтрация. Настройка форм поиска](https://kb.comindware.ru/article.php?id=5722)»* и *«[Таблицы. Расширенная фильтрация](https://kb.comindware.ru/article.php?id=5594#table_personal_use_filter_extended)»*.
...
На форму можно поместить вложенную форму из шаблона, связанного с текущим шаблоном. Это позволяет многократно использовать одну и ту же форму в различных контекстах. Например, вложенную форму рекомендуется использовать в качестве [стартовой формы **простого начального события** и формы **пользовательской задачи**](https://kb.comindware.ru/article.php?id=5660).

Подробные сведения см. в параграфе *«[Настройка представления вложенной формы](https://kb.comindware.ru/article.php?id=5723#form_dynamic_elements_embedded_form)»*.
...
1. В панели элементов:
   ...
   - наведите указатель на название **[атрибута типа запись](https://kb.comindware.ru/article.php?id=5718)** в списке элементов, чтобы создать атрибут в связанном с этим атрибутом шаблоне.
...
_![Создание атрибута с помощью панели элементов](/platform/v6.0/business_apps/templates/forms/img/form_designer_create_attribute.png)_
...
_![Переход к редактированию формы с помощью панели элементов](/platform/v6.0/business_apps/templates/forms/img/form_designer_edit_form.png)_
...
_![Создание кнопки с помощью панели элементов](/platform/v6.0/business_apps/templates/forms/img/form_designer_create_button.png)_

### Редактирование правил для формы

Подробные сведения см. в статье *«[Правила для формы. Определения, логика работы и настройка](https://kb.comindware.ru/article.php?id=5721)»*.
...
4. [Настройте правила](https://kb.comindware.ru/article.php?id=5721).

_![Переход к конструктору правил для формы](/platform/v6.0/business_apps/templates/forms/img/form_designer_goto_form_rules.png)_
...
- *[Статические элементы формы: области, вкладки, колонки, статичный текст. Настройка представления](https://kb.comindware.ru/article.php?id=5725)*
- *[Динамические элементы формы: поля атрибутов и вложенные формы. Настройка представления](https://kb.comindware.ru/article.php?id=5723)*
- *[Настройка представления вложенной формы](https://kb.comindware.ru/article.php?id=5723#form_dynamic_elements_embedded_form)*
