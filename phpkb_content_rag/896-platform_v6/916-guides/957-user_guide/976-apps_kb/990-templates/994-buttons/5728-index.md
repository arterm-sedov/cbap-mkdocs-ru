...
title: 'Кнопки'
kbId: 5728
url: 'https://kb.comindware.ru/article.php?id=5728'
updated: '2026-06-16 19:14:27'
---
...
- Названия, цвета и значки кнопок [настраиваются в **области кнопок**](https://kb.comindware.ru/article.php?id=5729).
...
Цвет и значок кнопки [настраиваются в **области кнопок**](https://kb.comindware.ru/article.php?id=5729).
...
_![Список кнопок в составе шаблона](/platform/v6.0/business_apps/templates/buttons/img/button_list.png)_

### Свойства

На вкладке «**Свойства**» можно настроить основные параметры кнопки.

_![Конструктор кнопки — вкладка «Свойства»](/platform/v6.0/business_apps/templates/buttons/img/button_designer.png)_

- **Отображаемое название** — подпись, которая будет отображаться на кнопке. Изменение отображаемого названия кнопки в [области кнопок](https://kb.comindware.ru/article.php?id=5729), на форме или в таблице не повлияет на название исходной кнопки в шаблоне.
- **Системное имя** — уникальное имя кнопки для использования в сценариях, выражениях и скриптах.
- **Описание** — наглядное описание назначения кнопки. Описание кнопки отображается как подсказка при наведении указателя мыши на кнопку. Изменение описания кнопки в [области кнопок](https://kb.comindware.ru/article.php?id=5729), на форме или в таблице не повлияет на описание исходной кнопки в шаблоне.
- **Контекст операции** — определяет набор доступных операций, контекст выполнения операции и возможность использования кнопки на соответствующих формах и таблицах.
  - **Аккаунт** — запись в **[шаблоне аккаунта](https://kb.comindware.ru/article.php?id=5695)**.
  - **Диаграмма** — диаграмма процесса.
  - **Запись** в **шаблоне [записи](https://kb.comindware.ru/article.php?id=5693)**, **[аккаунта](https://kb.comindware.ru/article.php?id=5695)**, **[роли](https://kb.comindware.ru/article.php?id=5692)** или **[организационной единицы](https://kb.comindware.ru/article.php?id=5697)**.
  - **Пользовательская задача** — элемент диаграммы процесса.
  - **Процесс** — экземпляр или **[шаблон процесса](https://kb.comindware.ru/article.php?id=5694)**.
- **Операция** — набор доступных операций зависит от выбранного [**контекста операции**](#operationContext). Операции выполняются с **записями** в **шаблонах [записей](https://kb.comindware.ru/article.php?id=5694)**. Предусмотрены следующие операции:
  ...
  - **Скрипт C#** — выполнение скрипта C#. Скрипт может выполнять любые действия с использованием API и библиотек, доступных в **Comindware Platform**. См. [*«Написание скриптов на языке C#. Скрипт операции»*](https://kb.comindware.ru/article.php?id=5211).
  ...
  - **Экспорт записи** — экспорт записи посредством [шаблона экспорта](https://kb.comindware.ru/article.php?id=5733) в формате DOCX, XLSX или PDF.
...
- **Результат выполнения** — выберите действие, которое необходимо осуществить после выполнения операции:
  ...
  Для операции «**Скрипт C#**» скачиваемый файл задаётся выходным параметром `File` скрипта. См. *«[Написание скриптов на языке C#. Скрипт операции](https://kb.comindware.ru/article.php?id=5211)»*.

  - **Навигация** — переход к объекту, указанному в поле «**Переход к**»:
    - **Диаграмма** — переход к диаграмме экземпляра процесса.
    - **Задача запущенного процесса** — переход к текущей \*\*[пользовательской задаче](https://kb.comindware.ru/article.php?id=5666).
    - **Предыдущая страница** — переход к предыдущей странице, открытой в браузере.
    - **Рабочий стол**— переход **[рабочему столу](https://kb.comindware.ru/article.php?id=5646)** для текущего аккаунта.
    ...
    - **Следующая задача в списке** — переход к следующей (по ID) **[пользовательской задаче](https://kb.comindware.ru/article.php?id=5666)**экземпляра процесса, назначенной текущему аккаунту.
    - **Список задач** — переход к странице «**[Мои задачи](https://kb.comindware.ru/article.php?id=5647)**» для текущего аккаунта.
    - **Таблица** — переход к таблице с записями шаблона или экземплярами процесса.
    - **Экземпляр процесса** — переход к [диаграмме экземпляра процесса](https://kb.comindware.ru/article.php?id=5658).

  Примечание

  Для операции «[**Скрипт C#**](#csharpScript)» объект навигации задаётся выходным параметром `NavigationResult` скрипта. См. *«[Написание скриптов на языке C#. Скрипт операции](https://kb.comindware.ru/article.php?id=5211)»*.
...
- **Отображать диалоговое окно** — если установлен этот флажок, при нажатии кнопки откроется предварительно настроенное окно подтверждения операции.
  - **Настроить диалоговое окно** — переход к [**конструктору формы**](https://kb.comindware.ru/article.php?id=5728) операции, на которое можно перетащить локальные переменные.

_![Конструктор диалогового окна операции с локальной переменной](/platform/v6.0/business_apps/templates/buttons/img/button_designer_dialogue_designer.png)_
...
Скрипт на С# можно использовать для операции «**Экспорт записи**» с целью подготовки данных для шаблона экспорта. Cм. [*«Настройка шаблона экспорта с использованием C#»*](https://kb.comindware.ru/article.php?id=5336) и [*«Подготовка файла шаблона экспорта»*](https://kb.comindware.ru/article.php?id=5730).
...
_![Конструктор кнопки — вкладка «Условие отображение» с формулой](/platform/v6.0/business_apps/templates/buttons/img/button_designer_display_condition.png)_

#### Примеры условий отображения кнопки

Ниже представлены примеры условий отображения кнопки на [языке формул Comindware](https://kb.comindware.ru/article.php?id=5215).

Примечание

При составлении выражений учитывайте [контекст](https://kb.comindware.ru/article.php?id=4892) их вычисления.

1. Формула для отображения кнопки на форме или в таблице шаблона записи при условии, что [логический атрибут](https://kb.comindware.ru/article.php?id=5700) имеет значение `true`:
...
2. Формула для отображения кнопки в таблице любого шаблона при условии, что в таблице выбрана хотя бы одна запись (то есть установлены флажки выбора в первом столбце таблицы):
   ...
   true
   ```
3. Формула для отображения кнопки на [стартовой форме или форме пользовательской задачи](https://kb.comindware.ru/article.php?id=5660) при условии, что логический атрибут записи, связанной с экземпляром процесса, имеет значение `true`:
...
В сценарии по нажатию кнопки локальные переменные будут доступны в объекте `dialogVariables`. См. *«[Переменные в сценарии](https://kb.comindware.ru/article.php?id=5656)»*.

Локальные переменные представляют собой [атрибуты](https://kb.comindware.ru/article.php?id=5706).

Пример использования локальной переменной см. в статье *«[Использование локальной переменной для массовой обработки записей](https://kb.comindware.ru/article.php?id=5368)»*.

Для каждой переменной можно задать:

- **Отображаемое название** — наглядное название переменной;
- **Тип данных** — для переменных предусмотрены [такие же типы, как для атрибутов](https://kb.comindware.ru/article.php?id=5706);
- **Формат отображения** — для переменных предусмотрены [такие же форматы отображения, как для атрибутов](https://kb.comindware.ru/article.php?id=5706);
...
- **Шаблон** — можно задать для переменных следующих типов: «**[Запись](https://kb.comindware.ru/article.php?id=5718)**», «**[Аккаунт](https://kb.comindware.ru/article.php?id=5704)**», «**[Роль](https://kb.comindware.ru/article.php?id=5720)**», «**[Организационная единица](https://kb.comindware.ru/article.php?id=5711)**»;
- **Хранить несколько значений** — этот флажок можно установить для переменных следующих типов: «**Файл**» (см. типы атрибутов «**[Документ](https://kb.comindware.ru/article.php?id=5716)**» и «**[Изображение](https://kb.comindware.ru/article.php?id=5707)**»), «**[Запись](https://kb.comindware.ru/article.php?id=5718)**», «**[Аккаунт](https://kb.comindware.ru/article.php?id=5704)**», «**[Роль](https://kb.comindware.ru/article.php?id=5720)**», «**[Организационная единица](https://kb.comindware.ru/article.php?id=5711)**».

_![Конструктор кнопки — вкладка «Локальные переменные»](/platform/v6.0/business_apps/templates/buttons/img/button_designer_local_variables.png)_

### Связи

На вкладке «**Связи**» отображается список всех объектов и ресурсов приложения, связанных с кнопкой.

_![Конструктор кнопки — вкладка «Связи»](/platform/v6.0/business_apps/templates/buttons/img/button_designer_relations.png)_
...
- *[Области кнопок. Определение, настройка, клонирование, удаление](https://kb.comindware.ru/article.php?id=5729)*