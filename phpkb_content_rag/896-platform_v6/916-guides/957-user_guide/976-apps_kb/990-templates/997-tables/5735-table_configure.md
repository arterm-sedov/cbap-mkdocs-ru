...
kbId: 5735
url: 'https://kb.comindware.ru/article.php?id=5735'
updated: '2025-12-01 14:43:12'
---
...
- [пользовательское](https://kb.comindware.ru/article.php?id=5594) — настраивается пользователем для своего аккаунта. Например, пользователь может скрыть и переместить столбцы, отфильтровать, отсортировать и сгруппировать строки, подсчитать итоги.

В этом разделе представлены инструкции по настройке таблицы в шаблоне. См. также [*«Настройка таблицы на форме»*](https://kb.comindware.ru/article.php?id=5723#form_dynamic_elements_table).

_![Вкладки с таблицами шаблона](/platform/v6.0/business_apps/templates/tables/img/table_definition_table_templates.png)_
...
3. Отобразится список таблиц, настроенных для шаблона, с их свойствами.
   ...
   ![Список таблиц в шаблоне](/platform/v6.0/business_apps/templates/tables/img/table_definition_tables_list.png)
...
5. Отобразится конструктор таблицы со следующими областями:
   ...
   **(1) Панель элементов** — содержит [атрибуты](https://kb.comindware.ru/article.php?id=5706), которые можно [поместить в качестве столбцов](#table_configure_columns) в таблицу, а также кнопки для добавления в область кнопок.
   ...
   - Чтобы создать атрибут в связанном с этим атрибутом шаблоне, наведите указатель на название **[атрибута типа запись](https://kb.comindware.ru/article.php?id=5718)** и нажмите кнопку «**Добавить атрибут**».
   ...
   **(5) Кнопки**
   ...
   ![Области конструктора таблицы](/platform/v6.0/business_apps/templates/tables/img/table_settings_constructor_areas.png)
...
- **Ручная загрузка данных** — установите этот флажок , чтобы при открытии таблица отображалась без данных. Пользователь сможет настроить внешний вид таблицы, а затем нажать кнопку «**Загрузить данные**».
  ...
  ![Таблица с ручной загрузкой данных](/platform/v6.0/business_apps/templates/tables/img/table_settings_data_load_button.png)
...
- **Отображение формы** — укажите способ отображения форм записей из таблицы:
  ...
  - **Карточки** — выберите представление карточек, которое будет использоваться для отображения данных таблицы в виде плиток. См. *«[Карточки](https://kb.comindware.ru/article.php?id=5734#cards_configure_assign_to_table)»*.
...
Настройка количества строк, отображающихся в таблицах

Чтобы задать количество записей, которые будут отображаться в таблицах, измените следующую директиву в файле конфигурации экземпляра ПО:
...
# Первый пункт будет использоваться по умолчанию.
queryPageResultRange: [50, 5000, 1000, 1000000000 ]
```
...
_![Настройка сортировки строк таблицы по двум столбцам](/platform/v6.0/business_apps/templates/tables/img/table_settings_sort_settings.png)_
...
_![Настройка группировки строк таблицы по двум столбцам](/platform/v6.0/business_apps/templates/tables/img/table_settings_group_settings.png)_
...
_![Настройка подсчета итогов по столбцу таблицы](/platform/v6.0/business_apps/templates/tables/img/table_settings_count_settings.png)_
...
_![Настройка фильтра с оператором «И» по двум столбцам таблицы](/platform/v6.0/business_apps/templates/tables/img/table_settings_and_settings.png)_
...
_![Добавление и настройка столбца таблицы](/platform/v6.0/business_apps/templates/tables/img/table_settings_column_settings.png)_
...
В каждой таблице предусмотрена одна [область кнопок](https://kb.comindware.ru/article.php?id=5729).

- Изначально для таблицы используется область кнопок, в [свойствах](https://kb.comindware.ru/article.php?id=5729#button_area_configure) которой установлен флажок «**По умолчанию для таблиц**». В стандартной области кнопок, например, у шаблона записи находятся кнопки «**Сохранить**» и «**Архивировать**», а у шаблона процесса — «**Создать**», «**Остановить процесс**», «**Мигрировать**» и «**Архивировать**».
...
- Кроме того, у каждой таблицы предусмотрены перечисленные ниже системные кнопки, которые нельзя добавить и удалить:
  ...
  - **Мои настройки** — раскрывающееся меню для настройки таблицы:
    - **Настроить внешний вид** — [персональная настройка](https://kb.comindware.ru/article.php?id=5594) отображения столбцов таблицы для текущего пользователя.
...
2. В панели элементов слева отобразится список [кнопок шаблона](https://kb.comindware.ru/article.php?id=5728).
...
5. Чтобы удалить все элементы из области кнопок, нажмите кнопку «**Очистить**» в панели «**Свойства области кнопок**».
   ...
   При изменении области кнопок таблицы в [списке областей кнопок](https://kb.comindware.ru/article.php?id=5729) будет создана новая область кнопок, которая будет использоваться только для этой таблицы. Исходная область кнопок не будет изменена.
6. Чтобы восстановить исходную область кнопок, используемую по умолчанию для таблиц, нажмите кнопку «**Восстановить исходные кнопки**» в панели «**Свойства области кнопок**». После восстановления исходных кнопок и сохранения таблицы созданная для неё особая область кнопок будет удалена из [списка областей кнопок](https://kb.comindware.ru/article.php?id=5729).
7. Нажмите кнопку «**Сохранить**», чтобы сохранить таблицу.

_![Настройка области кнопок таблицы](/platform/v6.0/business_apps/templates/tables/img/table_settings_buttons_area_settings.png)_
...
1. Откройте [шаблон процесса](https://kb.comindware.ru/article.php?id=5694).
...
7. Добавьте таблицу задач [на страницу «**Мои задачи**»](#table_configure_add_to_my_tasks).
   ...
   ![Создание таблицы задач](/platform/v6.0/business_apps/templates/tables/img/table_task_settings_create.png)

   Создание таблицы задач

## Добавление таблицы на страницу «Мои задачи»

1. Откройте [раздел навигации](https://kb.comindware.ru/article.php?id=5644), в который требуется добавить [таблицу](https://kb.comindware.ru/article.php?id=5735) из любого шаблона или [таблицу задач](#table_configure_tasks_view) из шаблона процесса.
...
5. Перетащите таблицу задач в подраздел «**Мои задачи**» на макете раздела навигации. Если этот подраздел отсутствует, перетащите группу «**Мои задачи**» из панели элементов на макет раздела навигации. См. [*«Добавление страницы «Мои задачи» в раздел навигации»*.](https://kb.comindware.ru/article.php?id=5647#my_tasks_page_configure_add_to_navigation)
...
_![Добавление таблицы задач на страницу «Мои задачи»](/platform/v6.0/business_apps/templates/tables/img/table_my_tasks_addition_add.png)_
...
_![Клонирование таблицы](/platform/v6.0/business_apps/templates/tables/img/table_clone_clone.png)_
...
- [Настройка таблицы на форме](https://kb.comindware.ru/article.php?id=5723#form_dynamic_elements_table)
- [Таблицы. Использование и персональная настройка](https://kb.comindware.ru/article.php?id=5594)
- [Настройка страницы «Мои задачи»](https://kb.comindware.ru/article.php?id=5647)
- [Просмотр и настройка свойств шаблона](https://kb.comindware.ru/article.php?id=5696)
