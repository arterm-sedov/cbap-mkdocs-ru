...
kbId: 5739
url: 'https://kb.comindware.ru/article.php?id=5739'
...
Например, с их помощью можно настроить автонумерацию записей, см. *[«Автонумерация записей с помощью формулы, C#-скрипта или выражения N3»](https://kb.comindware.ru/article.php?id=5194)*.

Помимо переменных приложения в **Comindware Platform** имеются системные и локальные переменные, которые применяются в сценариях, см. подробнее *[«Использование переменных в сценарии»](https://kb.comindware.ru/article.php?id=5656)*.
...
- Чтобы присвоить атрибуту значение переменной приложения с помощью C#-скрипта в сценарии, используйте действие «**Изменить значения скриптом**» со следующими свойствами:
  ...
  - **Значение:**
    ...
    ```
- Чтобы использовать значение переменной приложения в **Задаче-вызове сценария** используйте образец C#-скрипта из статьи [*«Автонумерация записей с помощью формулы, C#-скрипта или выражения N3»*](https://kb.comindware.ru/article.php?id=5194).

## Просмотр списка и настройка переменных приложения

1. На странице «**[Администрирование](https://kb.comindware.ru/article.php?id=5643)**» приложения выберите пункт «**Переменные**» *‌*.
...
_![Список переменных приложения](/platform/v6.0/business_apps/img/variable_list.png)_
...
_![Настройка переменной приложения](/platform/v6.0/business_apps/img/variable_properties.png)_
