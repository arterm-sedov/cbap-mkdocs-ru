...
kbId: 5655
url: 'https://kb.comindware.ru/article.php?id=5655'
updated: '2025-05-13 15:22:06'
---
...
_![Настройка свойств события «Нажатие кнопки»](/platform/v6.0/business_apps/scenarios/img/scenario_event_properties.png)_
...
Это событие и необходимое после него действие «[**Сменить контекст**](#scenario_actions_change_context)» настраиваются автоматически при создании сценария на входе в [**элемент диаграммы** процесса](https://kb.comindware.ru/article.php?id=5662).

**Тип**, **контекстный шаблон** и **элемент диаграммы** этого события не следует менять во избежание ошибок при выполнении сценария.

Чтобы настроить сценарий на входе в другой элемент диаграммы, воспользуйтесь [меню элемента](https://kb.comindware.ru/article.php?id=5657#process_diagram_call_element_menu) в конструкторе диаграммы процесса.
...
Это событие и необходимое после него действие «[**Сменить контекст**](#scenario_actions_change_context)» настраиваются автоматически при создании сценария на выходе из [**элемента диаграммы** процесса](https://kb.comindware.ru/article.php?id=5662).

**Тип**, **контекстный шаблон** и **элемент диаграммы** этого события не следует менять во избежание ошибок при выполнении сценария.

Чтобы настроить сценарий на выходе из другого элемента диаграммы, воспользуйтесь [меню элемента](https://kb.comindware.ru/article.php?id=5657#process_diagram_call_element_menu) в конструкторе диаграммы процесса.
...
_![Настройка общих свойств действия на вкладке «Дополнительно»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_common_properties.png)_
...
_![Настройка действия «Создать запись»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_create_record.png)_
...
_![Настройка действия «Дублировать запись»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_clone_record.png)_
...
_![Настройка действия «Сменить контекст»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_change_context.png)_
...
_![Настройка действия «Изменить значения атрибутов»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_change_attribute_values.png)_

#### Создать документ по шаблону

Это действие формирует файл посредством [шаблона экспорта](https://kb.comindware.ru/article.php?id=5733) и помещает ссылку на него в переменную. Затем созданный файл следует поместить в атрибут типа «**Документ**» с помощью действия «[**Прикрепить документ к атрибуту**](#scenario_actions_attach_document_to_attribute)».
...
- **Поместить документ в переменную**
  ...
  ![Настройка переменной для действия «Создать документ по шаблону»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_create_document_variable.png)
...
- **Шаблон экспорта** — настройте шаблон экспорта для формирования и заполнения файла значениями атрибута из [контекстного шаблона](#scenario_events_common_properties) (указан на вкладке «**Дополнительно**»).
  ...
  ![Создание нового шаблона экспорта для действия «Создать документ по шаблону»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_create_document_using_new_template.png)
  ...
  ![Выбор имеющегося шаблона экспорта для действия «Создать документ по шаблону»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_create_document_using_existing_template.png)
...
_![Настройка действия «Прикрепить документ к атрибуту»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_attach_document_to_attribute.png)_
...
- Для сценария по любым событиям, кроме входа и выхода токена, сообщение об ошибке отобразится во всплывающем уведомлении в браузере, в **журнале действий и ошибок** *‌* на информационной панели и в **[журналах событий](https://kb.comindware.ru/article.php?id=5614)** на вкладке «**Операции**».
...
С помощью этого действия можно осуществлять отладку сценариев и, например, устанавливать запрет на сохранение записей. См. *[«Отладка формул, выражений N3, сценариев и C#-скриптов»](https://kb.comindware.ru/article.php?id=5164)* и [*«Сценарий. Проверка данных записи перед сохранением»*](https://kb.comindware.ru/article.php?id=5382).
...
_![Настройка действия «Проверить результат выражения»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_validate_expression_result.png)_

_![Пример сообщения действия «Проверить результат выражения» в цепочке событий](/platform/v6.0/business_apps/scenarios/img/scenario_verify_expression_csharp_event_chain_message.png)_
...
_![Настройка действия «Проверить результат скрипта»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_check_script_result.png)_

_![Пример сообщения действия «Проверить результат скрипта» в цепочке событий](/platform/v6.0/business_apps/scenarios/img/scenario_verify_expression_csharp_event_chain_message.png)_
...
_![Настройка действия «Выполнить по условиям»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_execute_on_conditions.png)_
...
_![Настройка действия «Повторять по числовому счётчику»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_counter_loop.png)_
...
_![Настройка действия «Повторять по количеству объектов»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_object_loop.png)_
...
_![Настройка действия «Запустить процесс»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_execute_process.png)_
...
_![Настройка действия «Отправить сообщение»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_send_message.png)_
...
_![Настройка действия «Изменить значения переменных»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_change_variable_values.png)_
...
- *[Автонумерация записей с помощью формулы, C#-скрипта или выражения N3](https://kb.comindware.ru/article.php?id=5194)*
- *[Использование переменных приложения в C#-скриптах](https://kb.comindware.ru/article.php?id=5739#variables_csharp)*
...
_![Настройка действия «Изменить значения скриптом»](/platform/v6.0/business_apps/scenarios/img/scenario_actions_change_value_script.png)_

## Связанные статьи

- *[Сценарии. Определение, создание, настройка, использование](https://kb.comindware.ru/article.php?id=5654)*
- *[Использование переменных в сценарии](https://kb.comindware.ru/article.php?id=5656)*
- *[Сценарий. Проверка данных записи перед сохранением](https://kb.comindware.ru/article.php?id=5382)*
