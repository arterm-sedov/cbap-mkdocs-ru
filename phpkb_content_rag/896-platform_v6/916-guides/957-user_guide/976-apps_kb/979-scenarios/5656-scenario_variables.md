...
kbId: 5656
url: 'https://kb.comindware.ru/article.php?id=5656'
updated: '2026-06-09 16:57:49'
---
...
В сценариях можно обращаться к [системным переменным](#системные-переменные), [локальным переменным сценария](#локальные-переменные-сценария) и [переменным приложения](https://kb.comindware.ru/article.php?id=5739).
...
- `dialogVariables` — [локальные переменные кнопки](https://kb.comindware.ru/article.php?id=5728#button_local_variables), которая вызвала сценарий по событию «**Нажатие кнопки**». Значения локальных переменных кнопки задаются в диалоговом окне для кнопки. См. [пример](#практический-пример).
- `IncomingMessage` — в эту переменную можно поместить значения атрибутов ответа на HTTP-запрос в сценариях по событию «**Получено сообщение**».
  ...
  - См. пример в *«[HTTP-запросы. Получение JSON-данных в сценарии](https://kb.comindware.ru/article.php?id=5311)»*.
...
_![Результирующий сценарий](/platform/v6.0/business_apps/scenarios/img/scenario_with_variables.png)_
...
- *[Настройка действий сценария](https://kb.comindware.ru/article.php?id=5655)*