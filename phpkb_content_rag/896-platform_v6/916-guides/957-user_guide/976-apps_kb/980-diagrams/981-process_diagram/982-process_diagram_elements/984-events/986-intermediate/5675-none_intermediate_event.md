...
- **Временная веха** — задайте время после запуска экземпляра процесса, за которое токен должен дойти до данного события.
  ...
  - Это значение влияет на **текущее состояние процесса**, отображающееся в [списке его экземпляров](https://kb.comindware.ru/article.php?id=5638).
    ...
    - Если превышена [продолжительность выполнения процесса](../end/none_end_event.html#process-duration "Дополнительные"), заданная в каком-либо **[конечном событии](https://kb.comindware.ru/article.php?id=5681)**, процесс выполняется **с опозданием**.
