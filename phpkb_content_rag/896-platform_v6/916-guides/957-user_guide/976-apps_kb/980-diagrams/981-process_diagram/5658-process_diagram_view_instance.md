...
kbId: 5658
url: 'https://kb.comindware.ru/article.php?id=5658'
updated: '2025-11-12 16:09:38'
---
...
Диаграмма экземпляра процесса позволяет просмотреть текущие позиции токенов на элементах выполняющегося процесса, [создавать](#token_create "Создание токена"), [перезапускать и удалять токены](#tokens-tab), остановить процесс, просмотреть [цепочку событий](#events_chain_view) процесса.
...
3. Откройте [список версий](https://kb.comindware.ru/article.php?id=5659) диаграммы процесса.
...
2. **Сведения об экземпляре процесса**
   ...
   - **Свойства** — контекст экземпляра процесса:
     ...
     - **Версия** — ссылка на версию диаграммы, по которой был запущен экземпляр процесса.
       См. *«[Управление версиями диаграммы процесса](https://kb.comindware.ru/article.php?id=5659)»*.
   ...
   - **Журнал изменений** — список событий, произошедших в ходе выполнения процесса. В журнале изменений отображается подробная информация обо всех токенах и шагах процесса:
     ...
     - цепочка событий — нажмите значок рядом с элементом в журнале изменений, чтобы просмотреть цепочку событий для этого элемента.
       См. *«[Просмотр цепочки событий](#events_chain_view)»*.
...
4. **Позиции токенов** — элементы, на которых находятся токены, выделяются цветной рамкой:

   - зелёная — нет проблем;
   - жёлтая — превышен срок выполнения элемента;
   - красная — ошибка.

_![Диаграмма экземпляра процесса](/platform/v6.0/business_apps/diagrams/process_diagram/img/process_diagram_view_instance.png)_
...
2. Нажмите значок слева от события.
   ...
   ![Значок события в журнале изменений](/platform/v6.0/business_apps/diagrams/process_diagram/img/process_diagram_view_instance_event_icon.png)
...
_![Просмотр цепочки событий экземпляра процесса](/platform/v6.0/business_apps/diagrams/process_diagram/img/process_diagram_view_events_chain.png)_
