...
kbId: 5319
url: 'https://kb.comindware.ru/article.php?id=5319'
...
1. Откройте [список подключений](https://kb.comindware.ru/article.php?id=5303).
...
При SSO-аутентификации, если экземпляр ПО **Comindware Platform** работает под управлением ОС Linux, то для обеспечения возможности аутентификации новых пользователей, добавленных при синхронизации с сервером каталогов необходимо заново создать keytab-файл на сервере с экземпляром ПО.
...
См. параграф [*«Обновление keytab-файла аутентификации»*](https://kb.comindware.ru/article.php?id=5378#sso_authentication_configure_keytab_update).

_![Настройка основных свойств подключения к серверу каталогов](/platform/v6.0/administration/connections_communication_routes/authentication/img/ad_connection_properties_settings.png)_
...
_![Настройка групп на сервере каталогов](/platform/v6.0/administration/connections_communication_routes/authentication/img/ad_connection_groups_settings.png)_
...
_![Сопоставление атрибутов для подключения к серверу каталогов](/platform/v6.0/administration/connections_communication_routes/authentication/img/ad_connection_attributes_difference.png)_
...
- **Домен** — укажите домен сервера каталогов, используемый для аутентификации.
  ...
  - В поле «**Домен**» укажите домен, который будет использоваться как **домен по умолчанию** для данного подключения и (при настройке SSO через Kerberos в Linux) как домен по умолчанию для аутентификации. См. *«[SSO-аутентификация. Настройка ПО, контроллера домена и ПК пользователя](https://kb.comindware.ru/article.php?id=5378)»*.
...
_![Настройка дополнительных свойств подключения к серверу каталогов](/platform/v6.0/administration/connections_communication_routes/authentication/img/ad_connection_advanced_settings.png)_
...
- *[Удаление подключения](https://kb.comindware.ru/article.php?id=5303)*