...
kbId: 5635
url: 'https://kb.comindware.ru/article.php?id=5635'
updated: '2026-06-02 13:15:59'
---
...
Если внешняя система передаёт данные не через брокер сообщений, а прямым HTTP-запросом, используйте *«[Адаптер для импорта данных. Настройка подключения, пути передачи данных и сценария](https://kb.comindware.ru/article.php?id=5743)»*. Этот адаптер принимает HTTP-запрос, преобразует XML-сообщение во внутреннюю структуру DataImport и запускает сценарий обработки сообщения.

Если требуется отправлять данные из **Comindware Platform** в топик Apache Kafka, используйте *«[Адаптер экспорта данных из платформы. Настройка подключения, пути передачи данных и сценария](https://kb.comindware.ru/article.php?id=5744)»*.
...
- **Потребитель**, **консюмер** — это сервис, который получает сообщения из топиков Apache Kafka.
...
1. Откройте страницу [«**Администрирование**» — «**Пути передачи данных**»](https://kb.comindware.ru/article.php?id=5608).
...
3. Настройте параметры на вкладке «**Основные свойства**»:
   ...
   - **Подключение** — выберите ранее настроенное [подключение к шине Apache Kafka](#kafka_connection_configure): `kafka_receive_messages_connection`
...
4. Настройте параметры на вкладке «**Атрибуты сообщений**»:
   ...
   - **Запрос** — с помощью кнопки «**Добавить**» сформируйте структуру атрибутов, соответствующую структуре сообщения в формате XML/JSON. Значения этих атрибутов будут помещены в переменную и обработаны с помощью [сценария](#kafka_connection_scenario), срабатывающего при наступлении события «**Получение сообщения**»:
     ...
     - Для получения данных из приведённого выше сообщения создайте следующие атрибуты:
       ...
       | *orders* | *code* | **Строка** |  |
       | *orders* | *amount* | **Число** |  |
   - **Ответ** и **Ответ с ошибкой** — аналогично запросу, сформируйте структуру атрибутов для отправки серверу Apache Kafka ответа на полученный запрос. Значения этим атрибутам можно присвоить с помощью [сценария](#kafka_connection_scenario), срабатывающего при наступлении события «**Получение сообщения**».
...
Для получения сообщений через шину Apache Kafka и передачи данных в шаблон записи требуется настроить [сценарий](https://kb.comindware.ru/article.php?id=5654), срабатывающий при поступлении сообщения из Apache Kafka.

1. На странице **администрирования приложения** выберите пункт «**Сценарии**».
2. Создайте сценарий:

   - **Название**: *Получение данных из Apache Kafka*
...
_![Настроенный сценарий для получения данных через шину Apache Kafka](/platform/v6.0/administration/connections_communication_routes/message_queue_connections/../email_connections/img/kafka_connection_scenarios.png)_
...
- *[Адаптер экспорта данных из платформы. Настройка подключения, пути передачи данных и сценария](https://kb.comindware.ru/article.php?id=5744)*