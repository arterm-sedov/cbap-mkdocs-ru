...
kbId: 5628
url: 'https://kb.comindware.ru/article.php?id=5628'
...
Кроме того, потребуется настроить **[начальное](https://kb.comindware.ru/article.php?id=5671)** или **[промежуточное](https://kb.comindware.ru/article.php?id=5677) событие-получение сообщения** на диаграмме процесса.
...
_![Настройка основных свойств пути передачи данных для получения эл. почты в процессе](/platform/v6.0/administration/connections_communication_routes/email_connections/img/process_receiving_connection_settings_properties.png)_
...
3. Настройте передачу значений атрибутов сообщения в атрибуты записи с помощью вкладки «**Данные сообщения**» в свойствах [**события-получения сообщения**](https://kb.comindware.ru/article.php?id=5671), использующего этот путь передачи данных.
...
Подробные сведения об использовании JSONPath см. в статье *«[HTTP-запросы. Получение и обработка данных с помощью JSONPath](https://kb.comindware.ru/article.php?id=5323)»*.

_![Настройка атрибутов сообщения в пути передачи данных для получения эл. почты в процессе](/platform/v6.0/administration/connections_communication_routes/email_connections/img/process_receiving_connection_attributes_settings.png)_

## Настройка события-получения сообщения

1. Поместите на диаграмму процесса [начальное](https://kb.comindware.ru/article.php?id=5671) или [промежуточное](https://kb.comindware.ru/article.php?id=5677) событие-получение сообщения.

   ![Событие-получение сообщения на диаграмме процесса](/platform/v6.0/administration/connections_communication_routes/email_connections/img/process_receiving_connection_process_diagram.png)
...
5. На вкладке «**Данные сообщения**» сопоставьте **[атрибуты сообщения](#process_receiving_communication_route_properties_message_attributes)** с атрибутами **[шаблона записи](https://kb.comindware.ru/article.php?id=5693)**, связанного с шаблоном процесса:
   ...
   ![Сопоставление атрибутов сообщения из пути передачи данных с данными процесса](/platform/v6.0/administration/connections_communication_routes/email_connections/img/process_receiving_connection_message_data.png)
...
- *[Отправка, получение и обработка эл. почты в процессе. Пример: настройка подключений, путей передачи данных, диаграммы процесса и сценария](https://kb.comindware.ru/article.php?id=5375)*
- *[Отправка эл. почты из процесса. Настройка подключения, пути передачи данных и события на диаграмме процесса](https://kb.comindware.ru/article.php?id=5366)*
- *[Получение эл. почты с помощью сценария через Exchange и IMAP. Настройка подключения, пути передачи данных и сценария](https://kb.comindware.ru/article.php?id=5629)*
