...
kbId: 5629
url: 'https://kb.comindware.ru/article.php?id=5629'
...
Подключения типа «**Получение эл. почты через IMAP**» и «**Получение сообщений через Exchange**» используются в [сценариях](https://kb.comindware.ru/article.php?id=5654). В процессах используются подключения типа «**[Получение эл. почты в процессе](https://kb.comindware.ru/article.php?id=5628)**».
...
- С помощью [сценария](#scenario_receive_email_scenario_configure) передайте значения атрибутов сообщения, содержащие данные из эл. письма, в шаблон записи. Например, передайте текст и тему письма из атрибутов сообщения *HtmlBody* и *Subject* в атрибуты шаблона записи *«Текст письма»* и *«Тема письма»*.
...
Сценарий для получения почты может запускаться только по **[событию](https://kb.comindware.ru/article.php?id=5655#scenario_event)** «**Получение сообщения**».
...
_![Сценарий для получения электронной почты](/platform/v6.0/administration/connections_communication_routes/email_connections/img/scenario_receive_email_scenario.png)_
...
2. Перейдите к списку экземпляров [шаблона *«Реестр документов»*](#scenario_receive_email_template_configure).
3. В списке экземпляров откройте новую запись с временем создания несколько позже отправки письма на шаге 1.
   ...
   Время обработки входящих писем зависит от **интервала опроса**, заданного в [свойствах подключения к почтовому серверу](#scenario_receive_email_connection_configure), а также от загруженности почтового сервера и **Comindware Platform**.
...
- *[Получение эл. почты в процессе. Настройка подключения и пути передачи данных](https://kb.comindware.ru/article.php?id=5628)*
...
- *[Отправка эл. почты из процесса. Настройка подключения](https://kb.comindware.ru/article.php?id=5366)*
