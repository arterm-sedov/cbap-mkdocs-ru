...
kbId: 5424
url: 'https://kb.comindware.ru/article.php?id=5424'
...
В настоящем документе представлено краткое описание функций и возможностей **Comindware Platform**.

## Базовые функции Comindware Platform

### Возможности разработки приложений

**Comindware Platform** позволяет с помощью low-code-инструментов разрабатывать приложения для решения широкого спектра бизнес-задач. Как правило, в рамках одного приложения реализуют набор бизнес-процессов, связанных между собой общей бизнес-задачей.
...
### Возможности исполнения приложений

Приложения в **Comindware Platform** строятся на основе исполняемых моделей бизнес-процессов, что даёт перечисленные ниже возможности.
...
- Мониторинг производительности подсистем **Comindware Platform**.

## Построение архитектуры данных приложений

### Реестры данных

Для описания бизнес-сущностей, хранения и обработки данных в **Comindware Platform** используются реестры данных (также называемые шаблонами).

Например, реестр «Заказы» позволяет определить поля заказа на поставку товаров и услуг, формировать и хранить заказы.

Для построения архитектуры реестров данных в **Comindware Platform** используются следующие инструменты:
...
## Проектирование графического интерфейса приложений

### Создание форм для ввода и отображения данных

**Comindware Platform** позволяет создавать экранные формы для ввода и отображения данных. Для создания форм предусмотрен удобный в использовании визуальный конструктор.
...
- **Таблица** — служит для отображения, редактирования и создания записей в табличном представлении. Помимо таблиц на форме в **Comindware Platform** также предусмотрено создание таблиц, отображающихся на отдельных страницах.
...
**Comindware Platform** позволяет создавать таблицы для отображения наборов данных. Для создания таблиц предусмотрен удобный в использовании визуальный конструктор.
...
**Comindware Platform** позволяет строить модели бизнес-способностей организации с помощью удобного визуального конструктора.
...
**Comindware Platform** позволяет моделировать бизнес-процессы с помощью диаграмм в нотации BPMN 2.0, по которым автоматически исполняются приложения, назначаются задачи исполнителям, передаются сообщения и обрабатываются данные.
...
В **Comindware Platform** предусмотрен визуальный конструктор сценариев для автоматизации обработки данных, взаимодействия с внешними службами и выполнения действий в рамках бизнес-процессов.
...
В **Comindware Platform** предусмотрен визуальный конструктор правил для автоматизации обработки данных в формах.
...
### Инструменты для вычислений, выборки и обработки данных

В **Comindware Platform** предусмотрены перечисленные ниже инструменты, позволяющие вычислять значения атрибутов, осуществлять выборку данных согласно бизнес-правилам, обрабатывать данные при информационном обмене с внешними системами.

- Формулы **Comindware Platform** — выражения в понятной для бизнес-пользователей нотации, аналогичной формулам Excel.
...
В **Comindware Platform** используется встроенная современная система управления базами данных (далее «СУБД»), поддерживающая как реляционное, так и нереляционное представление данных.

Приобретение дополнительных специализированных программных продуктов не требуется, так как СУБД входит в состав **Comindware Platform**.

**СУБД Comindware Platform предоставляет следующие возможности:**
...
## Архитектура Comindware Platform

**Comindware Platform** построен на основе современной архитектуры веб-приложений.

В целях простоты дальнейшей доработки и масштабирования **Comindware Platform**, все функциональные и технологические слои архитектуры **Comindware Platform** максимально отделены друг от друга:
...
Слой бизнес-логики **Comindware Platform** реализован на современном технологическом стеке и предоставляет следующие возможности:
...
- простое развертывание приложений и обновлений без необходимости привлечения специалистов Поставщика **Comindware Platform**.