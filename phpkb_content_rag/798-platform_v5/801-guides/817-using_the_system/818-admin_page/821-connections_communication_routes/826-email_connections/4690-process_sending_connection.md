...
kbId: 4690
url: 'https://kb.comindware.ru/article.php?id=4690'
...
На диаграмме процесса можно использовать [промежуточные](https://kb.comindware.ru/article.php?id=5676) и [конечные](https://kb.comindware.ru/article.php?id=5680) события для отправки электронных писем с данными бизнес-процесса.
...
Подробный пример настройки приложения для обмена данными посредством эл. почты см. в статье *«[Пример: согласование заявлений по эл. почте. Настройка подключений, путей передачи данных и диаграммы процесса](https://kb.comindware.ru/article.php?id=5375)»*.
...
3. Для отправки писем с уведомлениями о пользовательских задачах:
   ...
   - [Настройте путь передачи данных](https://kb.comindware.ru/article.php?id=5624) типа «**Уведомления о задачах**».

## Настройка подключения типа «Отправка эл. почты из процесса»

Подключения типа «**Отправка эл. почты из процесса**» используются в **[промежуточных событиях-отправки сообщений](https://kb.comindware.ru/article.php?id=5676)**, **[конечных событиях-отправки сообщений](https://kb.comindware.ru/article.php?id=5680)**.

1. На странице **[«Администрирование» — «Подключения»](https://kb.comindware.ru/article.php?id=5608)** создайте или откройте подключение типа «**Отправка эл. почты из процесса**».
2. Настройте свойства подключения:

   - **Отключить** — установите этот флажок, чтобы временно деактивировать подключение.
   - **Использовать по умолчанию** — установите этот флажок, чтобы использовать подключение для отправки [**уведомлений о пользовательских задачах**](https://kb.comindware.ru/article.php?id=5624).
...
4. Сохраните подключение.
   ...
   ![Свойства подключения для отправки эл. почты](/platform/v5.0/administration/connections_communication_routes/email_connections/../../../examples/img/outgoing_email_configure_connection.png)

   Свойства подключения для отправки эл. почты

## Настройка пути передачи данных типа «Отправка эл. почты из процесса»

Пути передачи данных типа «**Отправка эл. почты из процесса**» используются в [промежуточных событиях-отправки сообщений](https://kb.comindware.ru/article.php?id=5676) и [конечных событиях-отправки сообщений](https://kb.comindware.ru/article.php?id=5680).

Путь передачи данных для отправки эл. почты служит для преобразования и передачи данных из **Comindware Platform** в эл. письмо.
...
_![Настройка основных свойств пути передачи данных для отправки эл. почты из процесса](/platform/v5.0/administration/connections_communication_routes/email_connections/../../../examples/img/outgoing_email_configure.png)_
...
_![Атрибуты сообщения — путь передачи данных для отправки эл. почты из процесса](/platform/v5.0/administration/connections_communication_routes/email_connections/../../../examples/img/outgoing_email_configure_message_attributes.png)_
...
_![Свойства сообщения — путь передачи данных для отправки эл. почты из процесса](/platform/v5.0/administration/connections_communication_routes/email_connections/../../../examples/img/outgoing_email_configure_message_properties.png)_

## Настройка события-отправки сообщения

1. Поместите на диаграмму процесса [промежуточное](https://kb.comindware.ru/article.php?id=5676) или [конечное](https://kb.comindware.ru/article.php?id=5680) событие-отправку сообщения.
...
2. В [меню элемента](https://kb.comindware.ru/article.php?id=5657#process_diagram_call_element_menu) события нажмите кнопку «**Свойства**» *‌*.
3. На вкладке «**Основные**» настройте [общие свойства элемента](https://kb.comindware.ru/article.php?id=5661).
4. На вкладке «**Дополнительные**» выберите путь передачи данных.

   - В разделе «**Место назначения**» выберите пункт «**Внешний сокет**».
   - С помощью раскрывающегося списка «**Использовать путь передачи данных**» выберите или создайте **[путь передачи данных](#process_sending_connection_communication_route_configure)** типа «**Отправка эл. почты из процесса**».![Выбор пути передачи данных для события-отправки сообщения](/platform/v5.0/administration/connections_communication_routes/email_connections/../../../examples/img/outgoing_email_configure_send_message_event_advanced_properties.png)
...
5. На вкладке «**Данные сообщения**» сопоставьте **[атрибуты сообщения](#process_sending_connection_message_attributes)** с атрибутами **шаблона записи**.
   ...
   - Значения указанных атрибутов **шаблона записи** будут присвоены **атрибутам сообщения** и переданы в [эл. письмо](#process_sending_connection_message_properties).![Сопоставление атрибутов сообщения из пути передачи данных с данными процесса](/platform/v5.0/administration/connections_communication_routes/email_connections/../../../examples/img/outgoing_email_configure_send_message_event_message_data.png)
...
- *[Подключения. Типы, создание, настройка, удаление](https://kb.comindware.ru/article.php?id=5303)*
- *[Пути передачи данных. Типы, создание, настройка, удаление](https://kb.comindware.ru/article.php?id=5302)*
...
- *[Общие уведомления. Настройка пути передачи данных, диаграммы процесса и глобальной конфигурации](https://kb.comindware.ru/article.php?id=5625)*
- *[Уведомления о задачах. Настройка пути передачи данных](https://kb.comindware.ru/article.php?id=5624)*
- *[Пример: согласование заявлений по эл. почте. Настройка подключений, путей передачи данных и диаграммы процесса](https://kb.comindware.ru/article.php?id=5375)*
- *[Получение эл. почты с помощью сценариев через IMAP и Exchange. Настройка подключения, пути передачи данных и сценария](https://kb.comindware.ru/article.php?id=5629)*
