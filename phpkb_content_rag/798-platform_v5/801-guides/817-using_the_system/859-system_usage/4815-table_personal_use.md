...
kbId: 4815
url: 'https://kb.comindware.ru/article.php?id=4815'
...
Сведения о настройке таблиц для разработчиков приложений см. в статьях *«[Настройка таблицы в шаблоне](https://kb.comindware.ru/article.php?id=4800#table_configure_template)»* и *«[Динамические элементы формы: настройка таблицы](https://kb.comindware.ru/article.php?id=4785#form_dynamic_elements_table)».*

## Элементы управления для таблицы

При просмотре таблицы конечному пользователю доступны различные элементы управления в зависимости от его [разрешений](https://kb.comindware.ru/article.php?id=4664):

1. [Кнопки](https://kb.comindware.ru/article.php?id=4790), настроенные для конкретной таблицы.
2. Стандартные кнопки для таблицы
   ...
   - **Мои настройки** *‌*
     ...
     - **Настроить таблицу** — переход к [конструктору таблицы](https://kb.comindware.ru/article.php?id=4800#table_configure_template).
     - **Настроить шаблон** — переход к странице свойств [шаблона](https://kb.comindware.ru/article.php?id=4709), к которому относится текущая запись.
...
Инструкции по настройке форм поиска: *[Расширенная фильтрация. Настройка форм поиска](https://kb.comindware.ru/article.php?id=5069)*.
...
1. Для **сортировки** значений таблицы нажимайте текст заголовка столбца. При каждом нажатии будет циклически меняться порядок сортировки:
   ...
   - исходный порядок, заданный в приложении (см. *«[Настройка таблицы в шаблоне](https://kb.comindware.ru/article.php?id=4800#table_configure_template)»)*.
...
1. Чтобы вернуть исходный внешний вид таблицы (порядок и видимость столбцов, закрепление столбцов, способ отображения формы записей, режим загрузки данных) с сохранением остальных персональных настроек (ширины столбцов, фильтров, сортировки, группировки, сортировки):
   ...
   5. Нажмите кнопку «**Сохранить**».![Сброс внешнего вида таблицы](/platform/v5.0/using_the_system/img/table_personal_use_drop_settings.png)
...
- *[Настройка таблицы в шаблоне](https://kb.comindware.ru/article.php?id=4800#table_configure_template)*
