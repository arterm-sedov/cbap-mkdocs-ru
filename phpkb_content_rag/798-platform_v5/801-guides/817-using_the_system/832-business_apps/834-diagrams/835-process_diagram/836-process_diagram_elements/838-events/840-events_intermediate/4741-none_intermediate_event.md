...
- **Временная веха** — задайте время после запуска экземпляра процесса, за которое токен должен дойти до данного события.
  ...
  - Это значение влияет на **текущее состояние процесса**, отображающееся в [списке его экземпляров](https://kb.comindware.ru/article.php?id=4709).
    ...
    - Если превышена [продолжительность выполнения процесса](https://kb.comindware.ru/article.php?id=4743#process-duration), заданная в каком-либо **[конечном событии](https://kb.comindware.ru/article.php?id=4744)**, процесс выполняется **с опозданием**.
