...
kbId: 4810
url: 'https://kb.comindware.ru/article.php?id=4810'
...
- Страницы можно включить в состав [рабочего стола](https://kb.comindware.ru/article.php?id=4813).

## Настройка свойств и макета страницы

1. Откройте [раздел навигации](https://kb.comindware.ru/article.php?id=4809).
2. Создайте или откройте страницу для редактирования:

   - Нажмите кнопку «**Создать**» *‌* рядом с пунктом «**Страницы**» на панели элементов.
   - Раскройте пункт «**Страницы**» в списке элементов и нажмите кнопку «**Редактировать**» *‌* рядом с пунктом с названием страницы.![Создание информационной страницы](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_add.png)

   Создание информационной страницы
3. Отобразится [конструктор страницы](#pages_setup_designer "Использование конструктора страницы").
...
5. Настройте [элементы](#pages_setup_elements "Настройка элементов страницы") и перетащите их на макет страницы с панели элементов.
6. Чтобы удалить элементы со страницы, перетащите их за пределы макета или нажмите кнопку «Очистить» и подтвердите удаление. Сохраните страницу.
   ...
   ![Добавление элемента на страницу](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_edit.png)
...
2. **Панель элементов слева** — список виджетов, которые можно перетащить на страницу.
   ...
   - **[Область](#pages_setup_area)** — содержит остальные элементы страницы.
   - **Вкладки**, **Колонки**, **Статичный текст** — эти элементы на страницах используются так же, как на формах. См. статью «[Статические элементы формы: области, вкладки, колонки, статичный текст. Настройка представления](https://kb.comindware.ru/article.php?id=4787)».
   - **[Карусель](#pages_setup_carousel_tiles)** — в этом элементе располагаются **плитки**.
   - **[Баннеры](#pages_setup_banner)**
   - **[Витрины](#pages_setup_showcase)**
   - **[Временные шкалы](#pages_setup_timeline)**
   - **[Диаграммы](#pages_setup_diagram)**
   - **[Плитки](#pages_setup_carousel_tiles)**
   - **[Показатели](#pages_setup_metrics)**
   - **[Системные виджеты](#pages_setup_system_widgets)**

     - **Добро пожаловать**
     - **Мои активные задачи**
     - **Последние события**
   - **[Списки](#pages_setup_list)**
   - **[Шаги](#pages_setup_steps)**
...
_![Конструктор страницы](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_designer.png)_
...
_![Добавление страницы в раздел навигации](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_add_page.png)_
...
Сведения о настройке доступа к страницам и виджетам см. в статье *[«Информационные страницы. Контроль доступа»](https://kb.comindware.ru/article.php?id=4811)*.

_![Переход к редактированию элемента страницы](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_element_edit.png)_
...
_![Настройка области](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_area_setting.png)_
...
_![Настройка баннера](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_banner_setting.png)_
...
3. Настройте витрину с помощью панели «**Свойства виджета**»:
   ...
   - **Внешний вид действия** — выберите способ отображения кнопки на плитке:
     ...
     - **Ссылка**
     - **Кнопка**
   - **Действие** — выберите [кнопку](https://kb.comindware.ru/article.php?id=4790) из **шаблона**, которая должна отображаться на плитке.
...
_![Настройка витрины](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_showcase_setting.png)_
...
**Временная шкала** отображает данные из выбранного шаблона в графической форме, аналогичной [диаграмме Ганта](https://kb.comindware.ru/article.php?id=4891). Конечный пользователь может нажимать элементы на временной шкале, чтобы перейти к соответствующим записям.

Для формирования временной шкалы потребуется создать шаблон записи (шаблон работ) с атрибутами указанных ниже типов.

Если в приложении уже настроена диаграмма Ганта, для построения временной шкалы можно использовать шаблон работ, используемый для построения диаграммы Ганта. См. статьи *«[Диаграмма Ганта. Настройка](https://kb.comindware.ru/article.php?id=4891)»* и *«[Диаграмма Ганта. Использование](https://kb.comindware.ru/article.php?id=4817)».*

#### Настройка шаблона работ

1. Создайте **[шаблон записи](https://kb.comindware.ru/article.php?id=4759)** *«Работы».*
...
_![Настройка временной шкалы](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_timeline_setting.png)_
...
3. Настройте **свойства виджета**:
   ...
   - **Тип** — выберите тип диаграммы:
     ...
     - **Гистограмма**
       ...
       - **Воронка** — отобразить горизонтальную гистограмму в виде воронки с убывающими сверху вниз столбцами. Чем выше показатель, тем шире столбец. Доступно только для горизонтальной гистограммы.

         Порядок настройки воронки

         Чтобы включить отображение воронки, задайте последовательно:

         1. **Тип** — выберите **Гистограмма**.
         2. **Горизонтальная** — установите флажок.
         3. **Воронка** — установите флажок.

         Пока не выполнены шаги 1 и 2, флажок «Воронка» недоступен.
   ...
   - **Сортировать по** — выберите способ упорядочивания категорий на диаграмме:

     - **Убыванию значения** — упорядочить категории по убыванию вычисленного показателя.
     - **Возрастанию значения** — упорядочить категории по возрастанию вычисленного показателя.
     - **Возрастанию категории** — упорядочить категории по возрастанию значения атрибута категорий.
     - **Убыванию категории** — упорядочить категории по убыванию значения атрибута категорий.

     Пример сортировки на гистограмме

     На гистограмме, сгруппированной по атрибуту *Статус* с параметром **Вычислять** = **Количество**:

     | Статус (Категории) | Количество (Значение) |
     | --- | --- |
     | Новая | 12 |
     | В работе | 7 |
     | Завершена | 25 |

     - **Убывание значения** → Завершена (25), Новая (12), В работе (7)
     - **Возрастание значения** → В работе (7), Новая (12), Завершена (25)
     - **Возрастание категории** → В работе, Завершена, Новая (по алфавиту)
     - **Убывание категории** → Новая, Завершена, В работе (в обратном порядке)
...
_![Настройка диаграммы](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_diagram_setting.png)_
...
_![Настройка плитки](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_tile_setting.png)_
...
_![Настройка показателя](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_metric_setting.png)_
...
- **Добро пожаловать** — стандартный баннер **Comindware Platform** содержащий ссылки на следующие разделы:
  ...
  - **Управление лицензиями** — переход к странице «[**Лицензирование**](https://kb.comindware.ru/article.php?id=4670)».
...
- **Активные задачи** — переход к странице «**[Мои задачи](https://kb.comindware.ru/article.php?id=4825)**».
- **Последние события** — недавние общие уведомления и уведомления о задачах. Для того чтобы отображались последние события, должны быть включены уведомления в **[Глобальной конфигурации](https://kb.comindware.ru/article.php?id=4668)**.
...
_![Настройка списка](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_list_setting.png)_
...
_![Настройка шагов](/platform/v5.0/business_apps/navigation_sections/img/pages_setup_steps_setting.png)_
...
- *[«Контроль доступа к информационным страницам»](https://kb.comindware.ru/article.php?id=4811)*