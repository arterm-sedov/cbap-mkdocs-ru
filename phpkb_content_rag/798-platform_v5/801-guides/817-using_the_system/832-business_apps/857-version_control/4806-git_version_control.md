...
Для работы этой функции требуется настроить подключение к репозиторию. См. статью *«[Управление версиями через Git. Настройка подключения](https://kb.comindware.ru/article.php?id=4680)»*.

Примечание

По умолчанию в репозиторий экспортируется только конфигурация приложения, записи сохраняются только для шаблонов, у которых установлен флажок «**Переносить данные при трансфере**» на вкладке [«**Свойства**»](https://kb.comindware.ru/article.php?id=4759#record_template_properties).
...
1. На странице **«[Администрирование](https://kb.comindware.ru/article.php?id=4714)»** приложения выберите пункт «**Управление версиями**» *‌*.
...
4. Выполните необходимые операции:
   ...
   - **Экспортировать в Git** — [экспорт](#version_control_git_export) текущей версии приложения в ветку, выбранную при [инициализации](#version_control_git_initialization) репозитория Git.![Операции на странице «Управление версиями через Git»](/platform/v5.0/business_apps/version_control/img/version_control_git.png)
...
Перед этим необходимо [настроить подключение к репозиторию](https://kb.comindware.ru/article.php?id=4680).

1. Нажмите кнопку «**Инициализация**» на странице «**Управление версиями через Git**».
2. Отобразится окно «**Инициализация репозитория Git**».
3. **Выберите репозиторий** — одно из настроенных [подключений к Git](https://kb.comindware.ru/article.php?id=4680).
...
10. Отобразится список коммитов (версий приложения) в выбранной ветке со следующими сведениями:
    ...
    - **Коммит** — уникальный идентификатор версии приложения в репозитории.![Список версий приложения в выбранной ветке репозитория](/platform/v5.0/business_apps/version_control/img/git_version_control_branch_list.png)
...
11. При необходимости нажмите кнопку «**Создать новую ветвь**».
    ...
    - нажмите кнопку «**Сохранить**».![Создание новой ветки в репозитории Git](/platform/v5.0/business_apps/version_control/img/git_new_branch.png)
...
6. Если экспорт не удаётся выполнить:
   ...
   - экспортируйте приложение заново.![Список ошибок экспорта приложения](/platform/v5.0/business_apps/version_control/img/git_version_control_export_errors.png)
...
6. Если импорт версии приложения не удаётся выполнить:
   ...
   - повторите импорт.![Список ошибок импорта приложения](/platform/v5.0/business_apps/version_control/img/git_version_control_import_errors.png)
