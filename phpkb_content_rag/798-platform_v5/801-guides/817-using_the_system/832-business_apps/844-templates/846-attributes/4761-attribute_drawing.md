...
См. также *«[Чертёж. Подготовка и импорт файла DWG](https://kb.comindware.ru/article.php?id=5371)»* и *«[Атрибут типа «Чертёж». Настройка шаблонов, атрибутов, форм и полей](https://kb.comindware.ru/article.php?id=5723#form_dynamic_elements_drawing)»*.
...
Имеется подготовленный чертёж в формате DWG с четырьмя слоями. Для корректного импорта чертёж необходимо подготовить в соответствии с требованиями, приведёнными в статье *«[Чертёж. Подготовка и импорт файла DWG](https://kb.comindware.ru/article.php?id=5371)»*.
...
| \_Запись \_ | *Этаж* | - **Связанный шаблон**: *Этажи* - **Хранить несколько значений**: флажок снят - **Взаимная связь с атрибутом:**   - **С новым**   - **Название**: *Помещения*   - **Хранить несколько значений**: флажок установлен |
...
3. Выберите блок «**Связи**» на поле чертежа.
   ...
   ![Настройка связей между слоями чертежа и шаблонами записи в приложении](/platform/v5.0/business_apps/templates/attributes/img/attribute_drawing_layers_settings.png)
...
5. Выберите блок «**Маркеры**» на поле чертежа.
   ...
   ![Настройка привязки маркеров на плане к записям в приложении и слоям чертежа](/platform/v5.0/business_apps/templates/attributes/img/attribute_drawing_markers_settings.png)
...
6. Настройте привязку записей из шаблона *«Заявки»* к маркерам на плане с помощью панели «**Свойства маркеров**»:
   ...
   - **Таблица:** *Все записи.* Эта таблица будет отображаться на вкладке «**Заявки**» слева от плана. Если для таблицы назначено [представление карточек](https://kb.comindware.ru/article.php?id=5734), то оно будет отображаться вместо таблицы.
   - **Привязка к слоям** — укажите для каждого уровня связи (слоя на чертеже) атрибут типа «**Запись**» из шаблона *«Заявки»*, ссылающийся на шаблон записи, заданный в **связях**. Маркеры будут привязаны к соответствующим слоям:
     ...
     - Слой *«Рабочие места»: атрибут* «Рабочее место»\_
7. Выберите блок «**Метаданные**» на поле чертежа.

   ![Выбор атрибутов, задающих цвета полигонов на плане](/platform/v5.0/business_apps/templates/attributes/img/attribute_drawing_polygon_colours.png)
...
10. Создайте и заполните записи в шаблонах *«Помещения»* и *«Рабочие места».*
    ...
    - Шаблон *«Рабочие места»*
      ...
      - *Создайте записи со следующими значениями атрибута* «ID рабочего места»\_:
...
13. С помощью мастера *«***Настройка плана***»* загрузите DWG-файл и привяжите слои полигонов и слои наименований на чертеже к шаблонам записи:
    ...
    ![Привязка слоёв чертежа к шаблонам записей в приложении](/platform/v5.0/business_apps/templates/attributes/img/attribute_drawing_layers_connect.png)
...
15. Поэкспериментируйте с планом:
    ...
    - выбирайте, скрывайте и отображайте слои, объекты и маркеры с помощью панели слева от чертежа.![Использование плана на форме](/platform/v5.0/business_apps/templates/attributes/img/attribute_drawing_plan_usage.png)

    Использование плана на форме

## Связанные статьи

- *[Чертёж. Подготовка и импорт файла DWG](https://kb.comindware.ru/article.php?id=5371)*
- *[Чертёж. Настройка поля на форме](https://kb.comindware.ru/article.php?id=5723#form_dynamic_elements_drawing)*