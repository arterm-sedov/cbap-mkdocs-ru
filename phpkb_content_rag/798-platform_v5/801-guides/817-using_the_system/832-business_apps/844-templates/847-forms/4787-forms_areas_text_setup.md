...
kbId: 4787
url: 'https://kb.comindware.ru/article.php?id=4787'
...
**Статические элементы** формы — это визуальные элементы. Их представление для конечного пользователя формируется с помощью [конструктора формы](https://kb.comindware.ru/article.php?id=4786) и помощью [правил для формы](https://kb.comindware.ru/article.php?id=4784) и не зависит от данных, отображающихся в форме.
...
- **Группа кнопок**, **Разделитель кнопок** — позволяют упорядочить кнопки в [области кнопок](https://kb.comindware.ru/article.php?id=4791#button_area_configure) на форме.
...
В каждой области предусмотрена [область кнопок](https://kb.comindware.ru/article.php?id=4791#button_area_configure), отдельная от основной области кнопок формы.
...
- **Системное имя** — уникальное имя элемента формы, используемое при настройке [правил для формы](https://kb.comindware.ru/article.php?id=4784).
...
- **Доступ** — режим доступа к данным полей внутри области и отображения области:
  ...
  - **Скрыть** — область и элементы внутри неё не будут отображаться на форме. Показать скрытую область можно с помощью [правил для формы](https://kb.comindware.ru/article.php?id=4784).
...
На каждой вкладке предусмотрена [область кнопок](https://kb.comindware.ru/article.php?id=4791#button_area_configure), отдельная от основной области кнопок формы.
...
- **Доступ** — режим доступа к данным полей внутри вкладок и отображения вкладок. Доступ можно настроить как для отдельных, так и для всех вкладок.
  ...
  - **Скрыть** — вкладки и элементы внутри них не будут отображаться на форме. Показать скрытый элемент можно с помощью [правил для формы](https://kb.comindware.ru/article.php?id=4784).
...
Каждая вкладка имеет собственное системное имя, и, следовательно, для каждой вкладки (как и для элемента в целом) имеется возможность настроить отдельное [правило](https://kb.comindware.ru/article.php?id=4784) отображения.
...
- **Доступ** — режим доступа к данным полей внутри колонок и отображения колонок. Доступ можно настроить как для отдельных, так и для всех колонок.
  ...
  - **Скрыть** — колонки и элементы внутри них не будут отображаться на форме. Показать скрытый элемент можно с помощью [правил для формы](https://kb.comindware.ru/article.php?id=4784).
...
- **Системное имя** — уникальное имя элемента формы, используемое при настройке [правил для формы](https://kb.comindware.ru/article.php?id=4784).
  Не должно начинаться с цифры. Разрешены английские и русские буквы, цифры и символ «\_». Рекомендуется использовать английские буквы.
  Обычно заполняется автоматически по названию.
- **Текст** — HTML-текст, который будет отображаться на форме. См. *«[Перечень поддерживаемых тегов и элементов HTML](https://kb.comindware.ru/article.php?id=4912)»*.
...
- *[Перечень поддерживаемых тегов и элементов HTML](https://kb.comindware.ru/article.php?id=4912)*
