...
kbId: 4786
url: 'https://kb.comindware.ru/article.php?id=4786'
updated: '2026-06-20 20:25:18'
---

# Формы. Определения и настройка
...
- На форме можно разместить поля, привязанные к атрибутам, вложенные формы, статичный текст, а также различные визуальные элементы. См.:
  - *[Динамические элементы формы: поля атрибутов и вложенные формы. Настройка представления](https://kb.comindware.ru/article.php?id=4785)»*
...
- Инструкции по использованию форм для пользователей см. в статье *«[Формы. Использование](https://kb.comindware.ru/article.php?id=4816)»*.

## Просмотр списка и настройка форм шаблона

Примечание

- Список форм предусмотрен у шаблонов [записей](https://kb.comindware.ru/article.php?id=4759), [аккаунтов](https://kb.comindware.ru/article.php?id=4757), [ролей](https://kb.comindware.ru/article.php?id=4760) и [организационных единиц](https://kb.comindware.ru/article.php?id=4755).
- В [шаблоне процесса](https://kb.comindware.ru/article.php?id=4758) список форм не предусмотрен, и формы создаются и настраиваются для элементов диаграммы процесса. См. статью *«[Стартовая форма и форма пользовательской задачи](https://kb.comindware.ru/article.php?id=4726)»*.
...
3. Настройте свойства формы:
   ...
   - **Тип**:
     ...
     - **Внутренняя** — выберите этот тип, чтобы форма отображалась, только если она вложена в другую форму или указана в качестве формы для записей (например, при запуске процесса, при создании записи, в диалоговых окнах). См. *«[Публичные и внутренние формы. Контроль доступа](https://kb.comindware.ru/article.php?id=4789)»*.
     - **Форма поиска** — выберите этот тип, чтобы настроить расширенную фильтрацию записей. См. *«[Расширенная фильтрация. Настройка форм поиска](https://kb.comindware.ru/article.php?id=5069)»* и *«[Таблицы. Расширенная фильтрация](https://kb.comindware.ru/article.php?id=4815#table_personal_use_filter_extended)»*.
...
На форму можно поместить вложенную форму из шаблона, связанного с текущим шаблоном. Это позволяет многократно использовать одну и ту же форму в различных контекстах. Например, вложенную форму рекомендуется использовать в качестве [стартовой формы **простого начального события** и формы **пользовательской задачи**](https://kb.comindware.ru/article.php?id=4726).

Подробные сведения см. в параграфе *«[Настройка представления вложенной формы](https://kb.comindware.ru/article.php?id=4785#form_dynamic_elements_embedded_form)»*.
...
1. В панели элементов:
   ...
   - наведите указатель на название **[атрибута типа запись](https://kb.comindware.ru/article.php?id=4780)** в списке элементов, чтобы создать атрибут в связанном с этим атрибутом шаблоне.
...
Подробные сведения см. в статье *«[Правила для формы. Определения, логика работы и настройка](https://kb.comindware.ru/article.php?id=4784)»*.
...
4. [Настройте правила](https://kb.comindware.ru/article.php?id=4784).
...
- *[Статические элементы формы: области, вкладки, колонки, статичный текст. Настройка представления](https://kb.comindware.ru/article.php?id=4787)*
- *[Динамические элементы формы: поля атрибутов и вложенные формы. Настройка представления](https://kb.comindware.ru/article.php?id=4785)*
- *[Настройка представления вложенной формы](https://kb.comindware.ru/article.php?id=4785#form_dynamic_elements_embedded_form)*
