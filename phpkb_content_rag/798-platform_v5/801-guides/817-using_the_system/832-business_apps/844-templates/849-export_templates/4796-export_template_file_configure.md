...
kbId: 4796
url: 'https://kb.comindware.ru/article.php?id=4796'
...
5. Должен получиться документ следующего вида (см. [пример](https://kb.comindware.ru/article.php?id=4795)):

_![Пример простейшего файла шаблона экспорта](/platform/v5.0/business_apps/templates/export_templates/img/export_template_file_example.png)_
...
- `{#LinkedTemplateAttribute}` — чтобы экспортировать значение атрибута типа «**Запись**» в виде гиперссылки на запись связанного шаблона, укажите системное атрибута с префиксом `#`. Гиперссылка будет содержать идентификатор или заголовок записи (если в шаблоне указан [атрибут-заголовок](https://kb.comindware.ru/article.php?id=4769)).
...
- `{ImageAttributeSystemName}` — если в поле указано системное имя атрибута типа «**[Изображение](https://kb.comindware.ru/article.php?id=4771)**», поле будет заменено на изображение, хранящееся в атрибуте.
- `{QRCodeAttributeSystemName:200:100}` — если в поле указано системное имя атрибута типа «**[Штрихкод](https://kb.comindware.ru/article.php?id=4764)**», поле будет заменено на изображение штрихкода, сформированного по значению атрибута. Для штрихкода можно указать ширину и высоту через двоеточие после системного имени атрибута. Если указать только ширину, то штрихкод будет квадратным. Если не указывать ширину и высоту, то будет использоваться стандартный размер штрихкода.

_![Пример синтаксиса полей шаблона экспорта](/platform/v5.0/business_apps/templates/export_templates/img/export_template_file_advanced_syntax_example.png)_

### Синтаксис для экспорта с использованием C#

Экспортировать данные посредством шаблона экспорта можно с помощью скрипта C#. Это позволяет более гибко настроить параметры экспорта, например с дополнительной фильтрацией, заменой информации, форматированным выводом [атрибутов типа «**Запись**»](https://kb.comindware.ru/article.php?id=4780) с несколькими значениями (коллекций).

Скрипт C# для шаблона экспорта следует ввести на вкладке «**Скрипт**» [кнопки экспорта](https://kb.comindware.ru/article.php?id=4798), которая автоматически создаётся для каждого шаблона экспорта.

Примеры скриптов C# для экспорта данных см. в статьях *«[Настройка шаблона экспорта с использованием C#](https://kb.comindware.ru/article.php?id=4793)»*, и *«[Настройка выгрузки нескольких коллекций и изображений](https://kb.comindware.ru/article.php?id=4792)»*
...
_![Пример XSLX-файла шаблона экспорта для C#](/platform/v5.0/business_apps/templates/export_templates/img/export_template_file_c_syntax_example_xlsx.png)_
...
_![Пример DOCX-файла шаблона экспорта для C#](/platform/v5.0/business_apps/templates/export_templates/img/export_template_file_c_syntax_example_docx.png)_

## Использование шрифтов в шаблонах экспорта в формате PDF

Если в [свойствах шаблона экспорта](https://kb.comindware.ru/article.php?id=4797) установлен флажок «**Экспортировать как PDF**», файлы PDF формируются с использованием шрифтов, установленных на сервере c экземпляром ПО **Comindware Platform** (далее «**Сервер**»).
