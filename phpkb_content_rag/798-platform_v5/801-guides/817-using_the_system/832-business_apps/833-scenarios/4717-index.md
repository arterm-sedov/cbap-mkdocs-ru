...
**Сценарий** представляет собой алгоритм автоматизации операций, проверки, обработки и передачи данных, который запускается при наступлении заданного [**события**](https://kb.comindware.ru/article.php?id=5655#scenario_event) и выполняет настроенную последовательность [**действий**](https://kb.comindware.ru/article.php?id=5655#scenario_actions).
...
См. [*«Событие и действия сценария. Определения, типы, свойства, настройка»*](https://kb.comindware.ru/article.php?id=5655) и [*«Использование переменных в сценарии»*](https://kb.comindware.ru/article.php?id=5656).
...
- Сценарий выполняется в контексте определённого шаблона записи. При этом [действия](https://kb.comindware.ru/article.php?id=5655#scenario_actions) «**Сменить контекст**», «**Создать запись**», «**Дублировать запись**» позволяют переключаться между шаблонами и манипулировать данными в них.
...
- Если при выполнении действия сценария происходит ошибка и в [свойствах этого действия](https://kb.comindware.ru/article.php?id=5655#scenario_actions_common_properties) не установлен флажок «**Игнорировать все ошибки**», то его действия отменяются полностью, как если бы сценарий не запускался вообще. Если ошибка сценария произошла в процессе, то процесс будет остановлен.
...
- Использование электронной подписи и подготовка документов по образцу
  - *[Обработка документов с электронной подписью](https://kb.comindware.ru/article.php?id=5367)*
  - *[Экспорт документов по шаблонам](https://kb.comindware.ru/article.php?id=5733)*
- Настройка бизнес-процессов
  - *[Визуализация этапов процесса с помощью шевронов](https://kb.comindware.ru/article.php?id=5379)*
  - *[Настройка универсального процесса согласования](https://kb.comindware.ru/article.php?id=5352)*
- Обмен данными с внешними системами
  - *[Отправка SQL-запроса SELECT во внешнюю СУБД](https://kb.comindware.ru/article.php?id=5374)*
  - *[Получение данных по таймеру из внешней СУБД](https://kb.comindware.ru/article.php?id=5347)*
  - *[Получение сообщений через Kafka](https://kb.comindware.ru/article.php?id=5635)*
- Получение и отправка HTTP-запросов
  ...
  - *[Отправка HTTP-запросов типа GET и обработка запросов в формате JSON](https://kb.comindware.ru/article.php?id=5312)*
- Получение и отправка электронной почты
  - *[Отправка эл. почты из сценариев через SMTP и Exchange](https://kb.comindware.ru/article.php?id=5630)*
  - *[Получение эл. почты из сценариев через SMTP и Exchange](https://kb.comindware.ru/article.php?id=5629)*
  - *[Согласование заявлений по эл. почте](https://kb.comindware.ru/article.php?id=5375)*
- Принудительная остановка бизнес-процессов и запрет на внесение изменений в записи
  - *[Проверка результата выражения в сценарии](https://kb.comindware.ru/article.php?id=5655#scenario_actions_validate_expression)*
  - *[Проверка результата скрипта в сценарии](https://kb.comindware.ru/article.php?id=5655#scenario_actions_validate_csharp)*
- Создание, дублирование и изменение записей
  - *[Копирование записи вместе с прикреплённым документом](https://kb.comindware.ru/article.php?id=5339)*

## Использование списка сценариев и создание сценария

1. Откройте страницу [«**Администрирование**» — «**Сценарии**»](https://kb.comindware.ru/article.php?id=5608).
...
_![Список сценариев в приложении](/platform/v5.0/business_apps/scenarios/img/scenarios_list.png)_
...
_![Настройка свойств нового сценария](/platform/v5.0/business_apps/scenarios/img/scenario_create_properties.png)_
...
- Нажмите кнопку «**Сценарий на входе**» *‌* или «**Сценарий на выходе**» *‌* в меню элемента [диаграммы бизнес-процесса](https://kb.comindware.ru/article.php?id=5657).

_![Кнопки создания сценария на входе и выходе из элемента диаграммы процесса](/platform/v5.0/business_apps/scenarios/img/scenario_exit_enter_process_element.png)_
...
_![Конструктор сценария](/platform/v5.0/business_apps/scenarios/img/scenario_designer.png)_
...
- **Перейти к контекстному шаблону** — нажмите кнопку *‌*, чтобы перейти к [шаблону](https://kb.comindware.ru/article.php?id=5638), в контексте которого выполняется элемент сценария.
...
_![Кнопки настройки элемента сценария](/platform/v5.0/business_apps/scenarios/img/scenario_element_buttons.png)_
...
5. [Настройте действие](#настройка-элемента-сценария).
   ...
   ![Создание действия в сценарии](/platform/v5.0/business_apps/scenarios/img/scenario_create_action.png)
...
_![Удаление действия из сценария](/platform/v5.0/business_apps/scenarios/img/scenario_delete_action.png)_
