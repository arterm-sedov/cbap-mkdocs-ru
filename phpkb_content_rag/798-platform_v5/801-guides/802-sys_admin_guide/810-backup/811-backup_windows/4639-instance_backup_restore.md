...
kbId: 4639
url: 'https://kb.comindware.ru/article.php?id=4639'
...
Сведения о создании файла резервной копии см. статьях *«[Резервное копирование и восстановление Comindware Platform. Краткое руководство для ОС Windows](https://kb.comindware.ru/article.php?id=4644)»*.
...
2. Если откроется страница настройки подключения к службе OpenSearch (Elasticsearch):

   1. В поле «URI» введите адрес своего сервера OpenSearch (Elasticsearch).
   2. Введите **префикс индекса**, **имя пользователя** и **пароль сервера OpenSearch (Elasticsearch)**.
   3. Нажмите кнопку «**Далее**».

   *[Настройка подключения к OpenSearch (Elasticsearch)](https://kb.comindware.ru/assets/Picture16.png)*
3. Если откроется страница инициализации данных в OpenSearch (Elasticsearch), нажмите кнопку «**Обновить**».

   ![Страница инициализации данных в OpenSearch (Elasticsearch)](https://kb.comindware.ru/assets/Picture17.png)

   Страница инициализации данных в OpenSearch (Elasticsearch)
4. Дождитесь открытия начальной страницы **Comindware Platform**.
5. Откройте страницу «**Администрирование**» — «**Подключения**».

   ![Переход к свойства подключения к OpenSearch (Elasticsearch)](https://kb.comindware.ru/assets/img_64d09fd6ec3ba.png)

   Переход к свойства подключения к OpenSearch (Elasticsearch)
6. Откройте подключение к серверу OpenSearch (Elasticsearch).
7. Удостоверьтесь, что корректно указаны **префикс индекса** и **URL подключения** к серверу OpenSearch (Elasticsearch).

   ![Проверка свойств подключения к серверу OpenSearch (Elasticsearch)](https://kb.comindware.ru/assets/img_64d0a41fc5e0b.png)

   Проверка свойств подключения к серверу OpenSearch (Elasticsearch)
...
10. Откройте страницу **[«Администрирование» — «Резервное копирование»](https://kb.comindware.ru/article.php?id=4642)**.
...
12. При необходимости измените конфигурации резервного копирования, указав корректные пути к файлам.
    ...
    ![Настройка пути к файлам резервной копии](https://kb.comindware.ru/assets/img_6683f69f9922d.png)

    Настройка пути к файлам резервной копии

## Связанные статьи

- *[Резервное копирование. Настройка и запуск Comindware Platform](https://kb.comindware.ru/article.php?id=4642)*
