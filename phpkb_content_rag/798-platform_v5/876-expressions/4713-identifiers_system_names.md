...
При [экспорте и импорте приложений](https://kb.comindware.ru/article.php?id=5652) идентификаторы назначаются заново, поэтому выражения, в которых они использовались, потребуется скорректировать соответствующим образом.
...
То есть системное имя шаблона может повторяться в разных приложениях, а системное имя атрибута может повторяться в различных шаблонах. Так, в каждом шаблоне есть [системные атрибуты](https://kb.comindware.ru/article.php?id=5717) `_creator` и `_creationDate`.

Системное имя задаёт создатель объекта. Оно является более наглядным, чем идентификатор.

При [экспорте и импорте приложений](https://kb.comindware.ru/article.php?id=5652) системные имена остаются неизменными, и корректировать выражения с ними не требуется.
...
- [Введение в вычисления](https://kb.comindware.ru/article.php?id=5182)
