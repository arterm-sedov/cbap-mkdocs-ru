...
title: 'Учебник по N3'
kbId: 5132
url: 'https://kb.comindware.ru/article.php?id=5132'
updated: '2026-06-20 18:06:05'
---
...
**Предусловие:** хорошее понимание принципов работы и настройки **Comindware Platform**.

## Типы синтаксиса запросов в Comindware Platform

В **Comindware Platform** предусмотрено два типа синтаксиса запросов:

1. **Формулы** — фирменный язык выражений **Comindware**, похожий на формулы Excel. Является простым для восприятия и представляет собой надстройку над **N3**. Для базовой настройки системы его зачастую достаточно. Типовые примеры использования формул:
   ...
   Полный перечень функций, используемых в **формулах**, представлен в [справочнике функций](https://kb.comindware.ru/category.php?id=901).
...
**Comindware Platform** использует графовую базу данных, основанную на модели RDF.
...
Триплет можно соотнести с простым предложением вида: *Подлежащее — Сказуемое — Дополнение*.

Например, утверждение *"Книга написана Пушкиным"* в RDF-терминологии можно представить следующим образом:
...
![Триплет RDF](/platform/v5.0/developer_guide/n3/img/n3_tutorial_triple.png)
...
Например, имеется шаблон записи «Книги». В шаблоне имеются атрибуты *«Название»* и *«Автор»*. В шаблоне имеются две записи.
...
Пример записей в шаблоне *«Книги»*:
...
3. **Неограниченная ширина и глубина связей между сущностями.** В **Comindware Platform** нет ограничений по количеству атрибутов и связей между ними.
...
Для лучшего понимания принципов описания и хранения данных без использования жёсткой структуры (такой, как таблицы в SQL), ознакомьтесь со статьёй *[«Онтология»](https://kb.comindware.ru/article.php?id=5254)*.

Внутренняя логика обработки N3 в Comindware Platform
...
- **Продвинутое использование Comindware Platform.**
- **Доступ ко всем созданным и системным атрибутам.** В интерфейсе **Comindware Platform** представлены не все системные атрибуты, а только те, которые используются чаще всего в типовых сценариях. N3 предоставляет доступ ко большему количеству атрибутов и сущностей, чем формулы.
- **Расширенные возможности работы с данными.** Дополнительные математические функции, функции для работы со списками, приведения данных к одному типу, расчётов времени и т.д. См. *[руководство по написанию выражений на N3](https://kb.comindware.ru/category.php?id=903)*.
...
- `?item` — входной параметр запроса. Его значение определяется контекстом, в котором выполняется запрос. Например, при вычислении значения атрибута `?item` содержит ID текущей записи. Подробные сведения см. в [Справочнике по языку N3](https://kb.comindware.ru/article.php?id=5251)/
...
Три шаблона записи. Первый шаблон имеет числовой атрибут *«Calculation»* и атрибут ссылку на второй шаблон. Второй ШЗ имеет ссылку на третий. В третьем ШЗ создаем атрибут *«Number»*. У шаблонов создаём по одной записи и устанавливаем ссылочные поля. В атрибут *«Number»* пишем число.

Задача: присвоить атрибуту *Calculation* значение атрибута *Number*.
...
**Библиотеки (префиксы)** — это подключаемые модули (области), где лежат знания об объектах, функциях и разных правилах. С их помощью мы можем обращаться с системным и прикладным атрибутам и брать их значения. Также, при объявлении библиотек может появляться доступ к различным функциям по работе с датами, списками, числами и т.д. См. [справочник по языку N3](https://kb.comindware.ru/article.php?id=5251).
...
*[RDF-триплет](img/n3_tutorial_rdf_triple.png)*
...
1. Первый способ, используемый в примере 1 урока. Субъект и предикат определен, значит искомое значение - это объект.
   ...
   ![](/platform/v5.0/developer_guide/n3/img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.008.png)
2. Триплет позволяет идти и от обратного, если известен объект и предикат, то мы находим субъект

   ![](/platform/v5.0/developer_guide/n3/img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.009.png)
...
![](img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.010.png)<figcaption></figcaption>
...
_![](/platform/v5.0/developer_guide/n3/img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.014.png)_6 строка — проверяем чтобы атрибут PlannedStartDate был пуст.
...
_![](/platform/v5.0/developer_guide/n3/img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.015.png)_Такое правило уже является «логическим или». Синтаксис условия or:
...
_![](/platform/v5.0/developer_guide/n3/img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.016.png)_Синтаксис: if {условие}
...
_![](/platform/v5.0/developer_guide/n3/img/Aspose.Words.3a78cd59-6370-458f-9e8c-c43e0d91fc2a.017.png)_В строке 8 и 9 мы использовали одноименную переменную для того, чтобы и в нее записалось объединенное значение.
