...
kbId: 2346
url: 'https://kb.comindware.ru/article.php?id=2346'
updated: '2024-11-29 16:59:22'
---
...
Это событие и необходимое после него действие «[**Сменить контекст**](#сменить-контекст)» настраиваются автоматически при создании сценария на входе в [**элемент диаграммы** процесса](https://kb.comindware.ru/article.php?id=2365).

**Тип**, **контекстный шаблон** и **элемент диаграммы** этого события не следует менять во избежание ошибок при выполнении сценария.

Чтобы настроить сценарий на входе в другой элемент диаграммы, воспользуйтесь [меню элемента](https://kb.comindware.ru/article.php?id=2360#process_diagram_call_element_menu) в конструкторе диаграммы процесса.
...
Это событие и необходимое после него действие «[**Сменить контекст**](#сменить-контекст)» настраиваются автоматически при создании сценария на выходе из [**элемента диаграммы** процесса](https://kb.comindware.ru/article.php?id=2365).

**Тип**, **контекстный шаблон** и **элемент диаграммы** этого события не следует менять во избежание ошибок при выполнении сценария.

Чтобы настроить сценарий на выходе из другого элемента диаграммы, воспользуйтесь [меню элемента](https://kb.comindware.ru/article.php?id=2360#process_diagram_call_element_menu) в конструкторе диаграммы процесса.
...
- Вкладка «**Дополнительно**»
  ...
  Флажок «**Сбрасывать кэш значений**» используется в **Comindware Business Application Platform** начиная с версии 4.7.3023.
...
Это действие формирует файл посредством [шаблона экспорта](https://kb.comindware.ru/article.php?id=2349) и помещает ссылку на него в переменную. Затем созданный файл следует поместить в атрибут типа «**Документ**» с помощью действия «[**Прикрепить документ к атрибуту**](#прикрепить-документ-к-атрибуту)».
...
- **Поместить документ в переменную**
  ...
  - **Переменная** — укажите имя переменной, в которую будет помещена ссылка на сформированный файл.![Настройка переменной для действия «Создать документ по шаблону»](https://kb.comindware.ru/assets/scenario_actions_create_document_variable.png)
...
- **Шаблон экспорта** — настройте шаблон экспорта для формирования и заполнения файла значениями атрибута из [контекстного шаблона](#scenario_event_common_properties) (указан на вкладке «**Дополнительно**»).
  ...
  - **Создать** — настройте новый шаблон экспорта:
    ...
    - **Экспортировать как PDF** — установите этот флажок, чтобы сформировать файл в формате PDF.![Создание нового шаблона экспорта для действия «Создать документ по шаблону»](https://kb.comindware.ru/assets/scenario_actions_create_document_using_new_template.png)

  Создание нового шаблона экспорта для действия «Создать документ по шаблону»

  - **Выбрать существующий**
    - **Выберите шаблон экспорта** — укажите шаблон экспорта из [контекстного шаблона](#scenario_event_common_properties) (указан на вкладке «**Дополнительно**»), по которому будет создан и заполнен файл.![Выбор имеющегося шаблона экспорта для действия «Создать документ по шаблону»](https://kb.comindware.ru/assets/scenario_actions_create_document_using_existing_template.png)
...
- Для сценария по любым событиям, кроме входа и выхода токена, сообщение об ошибке отобразится во всплывающем уведомлении в браузере, в **журнале действий и ошибок** *‌* на информационной панели и в **[журналах событий](https://kb.comindware.ru/article.php?id=2180)** на вкладке «**Операции**».
...
С помощью этого действия можно осуществлять отладку сценариев и, например, устанавливать запрет на сохранение записей. См. *[«Отладка формул, выражений N3, сценариев и C#-скриптов»](https://kb.comindware.ru/article.php?id=2636)* и [*«Сценарий. Запрет сохранения записи»*](https://kb.comindware.ru/article.php?id=2637).
...
- *[Сценарии. Определение, создание, настройка, использование](https://kb.comindware.ru/article.php?id=2151)*
- *[Использование переменных в сценарии](https://kb.comindware.ru/article.php?id=2399)*
- *[Сценарий. Проверка данных записи перед сохранением](https://kb.comindware.ru/article.php?id=2637)*
