---
name: phpkb-cloning
description: "Use when working on the PHPKB cloning and post-clone migration workflow in this repository: cloning PHPKB categories or articles, updating cloned PHPKB article/category links, migrating kbId frontmatter values to v5 IDs, fixing related topics after cloning, or analyzing/updating the root scripts phpkb_clone.py, phpkb_update_links.py, replace_related_topics.py, update_article_ids.py, and update_kbids_to_v5.py."
---
...
| `update_kbids_to_v5.py` | Migrate `kbId` frontmatter values in `docs/ru` using `.v5mapping.json`. | Rewrites Markdown files in place. |
...
2. Treat DB-mutating scripts as high risk.
   `phpkb_clone.py` and `phpkb_update_links.py` connect to PHPKB through `tools.ssh_kb_ru` and can insert or update production-like database records. Do not run them unless the user explicitly asks to execute the DB operation and understands the target.

3. Verify mapping files before link or ID migration.
   Check `.v5mapping.json`, `.mapping.json`, and article ID filename maps as relevant before using `phpkb_update_links.py` or `update_kbids_to_v5.py`.

4. Treat file-rewriting helpers as batch migrations.
   `replace_related_topics.py` and `update_kbids_to_v5.py` rewrite Markdown files in place. Before running them, check `git status --short`, inspect the search scope, and confirm it matches the requested files.
...
- Review `replace_related_topics.py`.
...
### Work With `update_article_ids.py`
